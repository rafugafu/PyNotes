...
if ctypes.windll.shell32.IsUserAnAdmin() == 0:
	...
	root = tk.Tk()
	root.title('PyNotes Installation Wizard')
	ttk.Label(master = root, text = 'Install PyNotes for Windows\nNB. This will add PyNotes to the PATH and Start Menu FOR ALL USERS.').grid(column = 0, row = 0, padx = 10, pady = 10, sticky = 'ew')
	...
	ttk.Label(master = root, text = 'Version (1.4.1 onwards):').grid(column = 0, row = 3, padx = 10, pady = 10, sticky = 'ew')
	versionno = tk.StringVar()
	ttk.OptionMenu(root, versionno, 'Latest', *['Latest', 'Custom'], command = lambda option: customver.grid_forget() if option == 'Latest' else [customver.grid(column = 0, row = 5, padx = 10, pady = 10, sticky = 'ew'), versionno.set(customver.get())]).grid(column = 0, row = 4, padx = 10, pady = 10, sticky = 'ew')
	customver = ttk.Entry()
