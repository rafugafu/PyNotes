...
try:
	...
	theme['python:variable_definitions']
	theme['python:operators']
	theme['python:function_definitions']
except:
...
def changes(event = None):
	...
	cw.text(master = cw_, text = '1. Added speech-to-text! (Alt + X - st or Options → Speech to Text to use it.)').grid(column = 0, row = 0, sticky = 'w')
	cw.text(master = cw_, text = '2. Improved rendering of code in MathGod.').grid(column = 0, row = 1, sticky = 'w')
...
def hpv():
	...
	while True:
		...
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		type_.tag_add('hpv', n, nn + '-1c')
		n = nn
	exec("type_.tag_config('hpv'," + theme['python:variable_definitions'] + ')')
def hpf():
	...
	while True:
		...
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		type_.tag_add('hpf', n + '+4c', nn + '-1c')
		n = nn
	exec("type_.tag_config('hpf'," + theme['python:function_definitions'] + ')')
def hpo():
	n = '1.0'
	searchstr = r'==|\!=|<|>|<=|>='
	while True:
