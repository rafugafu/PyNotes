...
def changes(event = None):
    ...
    cw.text(master = cw_, text = '1. Ask inputs in PyCode are now saved to the variable \'answer\' which can later be used!').grid(column = 0, row = 0, sticky = 'w')
    cw.text(master = cw_, text = '2. Made the python shell much much better!').grid(column = 0, row = 1, sticky = 'w')
    cw.text(master = cw_, text = '3. Added a new command to Alt-X: \'write:{string}*{n}\' or \'w:{string}*{n}\': copy the given text {n} number of times after the cursor.').grid(column = 0, row = 2, sticky = 'w')
    cw.text(master = cw_, text = '4. Changed the command \'hmode\' in Alt-X (see Help → Help with Alt-X.)').grid(column = 0, row = 3, sticky = 'w')
    cw.text(master = cw_, text = '5. Fixed a few minor bugs.').grid(column = 0, row = 4, sticky = 'w')
...
def helppycode():
    ...
    hpwin.text(master = kst, text = 'Remember, inputs in these should always be inside quotes, and single quotes only!\nWhen you ask something, the output is saved to the variable \'answer\'. If you have already defined a variable named \'answer\', the variable will not change. Also lines of code should always end with a ";". Here are a few examples:').grid(column = 0, row = 2)
