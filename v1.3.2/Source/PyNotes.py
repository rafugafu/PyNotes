...
def changes(event = None):
    ...
    cw.text(master = cw_, text = '1. Added UNDO and REDO! (I never knew it was that simple)').grid(column = 0, row = 0, sticky = 'w')
    cw.text(master = cw_, text = '2. Made the python shell much much better!').grid(column = 0, row = 1, sticky = 'w')
    cw.text(master = cw_, text = '3. The entire program does not freeze when speaking text anymore!').grid(column = 0, row = 2, sticky = 'w')
    cw.text(master = cw_, text = '4. Removed the spell-checking as it was extremely inefficient,\nand the entire program used to hang while opening any file above around 4 kb.\n(It may come back in later versions if I can find a way\n to make it more efficient and faster.)').grid(column = 0, row = 3, sticky = 'w')
    cw.text(master = cw_, text = '5. Fixed a few small bugs.').grid(column = 0, row = 4, sticky = 'w')
...
def pcread():
    ...
    global root
    pycodecommands = ['quit', 'say', 'command', 'selall', 'copy', 'paste', 'ask', 'cut', 'term', 'cmd', 'undo', 'redo']
    pythoncommands = ['root.destroy()', 'print', 'cmd()', 'selall()', 'cp()', 'pst()', 'input', 'cut()', 'term()', 'cmdrun', 'undo()', 'redo()']
...
def shellpy():
    ...
    def exec(event):
        ...
        if all((continuation == False, not command[-1:] == ':')):
            output = shellexec(codeforshellpy + command)
            if all((not output[0], re.findall(r'.+ += +.+', command))):
                codeforshellpy += command + '\n'
            output = output[1]
            shellcmd.insert('end', '\n' + output + '\n' + '>>> ')
            ...
            if not command == '':
                ...
                continuation = False
                output = shellexec(codeforshellpy + continuationcodeforshellpy)
                for line in continuationcodeforshellpy.split('\n'):
                    if all((not output[0], re.findall(r'.+ += +.+', line.strip()))):
                        codeforshellpy += line.strip() + '\n'
                output = output[1]
                shellcmd.insert('end', '\n' + output + '\n' + '>>> ')
                readonlyendforshellpy = shellcmd.index('end-1c').split('.')[0] + '.' + str(len('>>> '))
                readonlytextforshellpy += command + '\n' + output + '\n' + '>>> '
