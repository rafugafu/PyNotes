...
def changes(event = None):
    ...
    cw.text(master = cw_, text = '1. Added a terminal!').grid(column = 0, row = 0, sticky = 'w')
    cw.text(master = cw_, text = '2. Added an option to toggle regular backup').grid(column = 0, row = 1, sticky = 'w')
...
def keypress(event = None):
    global bfr
    if bfr and title and int(time.time() - start) % 3:
...
def term():
    if root.ask('Info', 'This is NOT a proper terminal. It can only do simple things, like "ls", or "echo". Do you still want to continue ?', options = ('ok', 'cancel')):
        tw = easytk.win()
        tw.title('Terminal')
        tf = tw.frame()
        ...
        in_ = tw.entry(master = tf)
        obf = tw.frame()
        scrolly = tw.scroll(master = obf)
        scrolly.pack(side = 'right', fill = 'y')
        out = tw.listbox(master = obf, yscrollcommand = scrolly.set)
        out.pack(fill = 'both')
        scrolly.config(command = out.yview)
        obf.pack(side = 'bottom', fill = 'x')
        in_.bind('<Return>', lambda event: [tw.sizabletrue(), os.system(in_.get() + ' > .termout')] + [out.insert('end', item) for item in open('.termout', 'r').read().split('\n')] + [in_.delete(0, 'end'), tw.sizablefalse(), os.remove('.termout')])
        in_.focus()
...
om.add_command(label = 'Source Code -> Ctrl + O', command = ss)
om.add_separator()
