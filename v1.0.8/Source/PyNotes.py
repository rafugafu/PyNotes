...
def changes(event = None):
    ...
    cw.text(master = cw_, text = '1. Added Emacs-like commands with Ctrl-X or menu!').grid(column = 0, row = 0, sticky = 'w')
    cw.text(master = cw_, text = '2. Improved the find option greatly').grid(column = 0, row = 1, sticky = 'w')
...
def cmdrun(command):
    ...
    if command == 'exit' or command == 'e':
        ...
        sssv()
    elif command == 'find' or command == 'search' or command == 'f':
        f()
