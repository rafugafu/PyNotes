...
def changes(event = None):
    ...
    cw.text(master = cw_, text = '1. Improved PyCode a lot!').grid(column = 0, row = 0, sticky = 'w')
    cw.text(master = cw_, text = '2. A few bug fixes').grid(column = 0, row = 1, sticky = 'w')
...
def termexec(string, listbox, window, bar, entry):
    ...
    if string == 'clear':
        ...
        listbox.insert('end', 'cannot run "cd" in this terminal')
        entry.delete(0, 'end')
    else:
        bar.config(value = 0)
        ...
        bar.grid_forget()
        try:
            result = subprocess.run(string, shell = True, text = True, capture_output = True)
        ...
        window.sizablefalse()
def term():
    root.warning('Warning!', 'Do not try to run commands which ask for an input or wait before execution!')
    tw = root.subwin()
    tw.title('Terminal')
    tf = tw.frame()
...
def spk(event = None):
    ...
    try:
        ...
        root.error('Error', 'There is some problem with the text-to-speech engine.')
    else:
        try:
            engine = stt.init()
