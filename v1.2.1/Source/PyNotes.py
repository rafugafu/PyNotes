...
def changes(event = None):
    ...
    cw.text(master = cw_, text = '1. Added variables in PyCode').grid(column = 0, row = 0, sticky = 'w')
    cw.text(master = cw_, text = '2. Starts with The Zen of Python now. (try "import this" in any python shell)').grid(column = 0, row = 1, sticky = 'w')
    cw.text(master = cw_, text = '3. Very greatly improved the help options. (Check Help → Help with PyCode)').grid(column = 0, row = 2, sticky = 'w')
...
def pcread():
    ...
    for line in code:
        ...
        if line:
            ...
            if ks and len(ks) == 1:
                ks = ks[0][:-1]
                cdt += f"root.bind('{ks.split('→')[0]}', lambda event: {ks.split('→')[1][:-1][1:]})" + '\n'
