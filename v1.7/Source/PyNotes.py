...
def fr(event = None):
	...
	def updatef():
		...
		if find:
			...
			try:
				while True:
					n = type_.search(find, n, nocase = case, stopindex = 'end', regexp = useregx, count = count)
					if not n: break
					nn = '%s+%dc' % (n, count.get())
					type_.tag_add('found', n, nn)
					foundlist.append((n, nn))
					n = nn
	...
	def updateff(event = None):
		if event and not event.keysym == 'Return' and not (event.state & 4):
			updatef()
		if not foundlist:
			return
		type_.tag_add('foundhighlight', foundlist[i][0], foundlist[i][1])
	...
	findbox.grid(column = 1, row = 0, padx = 10, pady = 10)
	ok.text(text = 'Replace:').grid(column = 0, row = 1, padx = 10, pady = 10)
	replacebox = ok.entry()
	if not emacskeysforsearch:
		replacebox.bind('<Return>', lambda event: fnext(replacebox.get()))
	replacebox.grid(column = 1, row = 1, padx = 10, pady = 10)
	cs = ok.booleanvar()
	ok.check(text = 'Case Sensitive', variable = cs, command = updateff).grid(column = 0, row = 2, padx = 10, pady = 10)
	regx = ok.booleanvar()
	ok.check(text = 'Use regexp', variable = regx, command = lambda: updateff).grid(column = 1, row = 2, padx = 10, pady = 10)
	ok.button(text = 'Previous', command = fback).grid(column = 0, row = 3, padx = 10, pady = 10)
	ok.button(text = 'Next', command = fnext).grid(column = 1, row = 3, padx = 10, pady = 10)
	ok.button(text = 'Replace', command = lambda: [type_.delete(foundlist[i][0], foundlist[i][1]), type_.insert(foundlist[i][0], replacebox.get())] if foundlist else None).grid(column = 0, row = 4, padx = 10, pady = 10)
	ok.button(text = 'Replace and next', command = lambda: fnext(replacebox.get())).grid(column = 1, row = 4, padx = 10, pady = 10)
	ok.button(text = 'Replace all', command = lambda: replaceall(replacebox.get())).grid(column = 0, row = 5, padx = 10, pady = 10)
	...
	ok.button(text = 'Close', command = lambda: [type_.tag_remove('found', '1.0', 'end'), type_.tag_remove('foundhighlight', '1.0', 'end'), ok.destroy(), type_.unbind('<KeyPress>')]).grid(column = 1, row = 5, padx = 10, pady = 10)
...
def f(event = None):
	...
	def updatef():
		...
		if find:
			...
			try:
				while True:
					n = type_.search(find, n, nocase = not case, stopindex = 'end', regexp = useregx, count = count)
					if not n: break
					nn = '%s+%dc' % (n, count.get())
					type_.tag_add('found', n, nn)
					foundlist.append((n, nn))
			...
			foundlist.clear()
	def updateff(event):
		if not event.keysym == 'Return' and not (event.state & 4):
			updatef()
		if not foundlist:
			return
		type_.tag_add('foundhighlight', foundlist[i][0], foundlist[i][1])
	...
	findbox.grid(column = 1, row = 0, padx = 10, pady = 10)
	cs = ok.booleanvar()
	ok.check(text = 'Case Sensitive', variable = cs, command = lambda: updateff).grid(column = 0, row = 1, padx = 10, pady = 10)
	regx = ok.booleanvar()
	ok.check(text = 'Use regexp', variable = regx, command = lambda: updateff).grid(column = 1, row = 1, padx = 10, pady = 10)
	ok.button(text = 'Previous', command = fback).grid(column = 0, row = 2, padx = 10, pady = 10)
	ok.button(text = 'Next', command = fnext).grid(column = 1, row = 2, padx = 10, pady = 10)
	type_.bind('<KeyPress>', lambda event: [type_.tag_remove('found', '1.0', 'end'), type_.tag_remove('foundhighlight', '1.0', 'end'), ok.destroy(), type_.unbind('<KeyPress>')])
	ok.button(text = 'Close', command = lambda: [type_.tag_remove('found', '1.0', 'end'), type_.tag_remove('foundhighlight', '1.0', 'end'), ok.destroy(), type_.unbind('<KeyPress>')]).grid(column = 0, row = 3, padx = 10, pady = 10)
	if emacskeysforsearch:
...
def prf(event = None):
	...
	f = pr.droptype(options = tuple(sorted(pr.getfonts())), master = mf, command = lambda: [pr.sizabletrue(), type_.config(font = f.get()), showfont.config(font = f.get()), pr.sizablefalse()])
...
def hpa():
	...
	for k in keyword.kwlist:
		...
		searchstr = r'\y' + k + r'\y'
		while True:
			n = type_.search(searchstr, n, nocase = 1, stopindex = type_bottom, regexp = True)
			if not n: break
			nn = '%s+%dc' % (n, len(k))
...
def hpc():
	...
	while True:
		count = root.intvar()
		n = type_.search(searchstr, n, nocase = 1, stopindex = type_bottom, regexp = True, count = count)
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		for tag in type_.tag_names():
			if tag == 'sel' or tag == 'marked' or tag == 'found' or tag == 'foundhighlight': continue
...
def hla():
	...
	while True:
		count = root.intvar()
		n = type_.search(search, n, nocase = 1, count = count, stopindex = type_bottom, regexp = True)
		if not n: break
		nn = '%s+%dc' % (n, count.get())
...
def hlb():
	...
	start = '1.0'
	while True:
		start = type_.search(r'\\begin{.+?}', start, nocase = 1, stopindex = type_bottom, regexp = True)
		if not start:
			break
		end = type_.search(r'\\end{.+?}', start, nocase = 1, stopindex = type_bottom, regexp = True)
		if not end:
			break
...
def hlh():
	...
	while True:
		count = root.intvar()
		n = type_.search(searchstr, n, nocase = 1, count = count, stopindex = type_bottom, regexp = True)
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		for tag in type_.tag_names():
			if tag == 'sel' or tag == 'marked' or tag == 'found' or tag == 'foundhighlight': continue
...
def hhstr():
	...
	while True:
		...
		nn = '%s+%dc' % (n, count.get())
		for tag in type_.tag_names():
			if tag == 'sel' or tag == 'marked' or tag == 'found' or tag == 'foundighlight': continue
			type_.tag_remove(tag, n, nn)
...
def type_setview():
	...
	ha(filetype.cget('text').replace('Python File (*.py)', 'py').replace('LaTeX / TeX File (*.tex)', 'latex').replace('HTML File (*.html)', 'html'))
	root.after(10, type_setview)
def do_backup():
	if all((not filetype.cget('text') in ['PNG Image (*.png)', 'PDF File (*.pdf)', 'EPUB File (*.epub)'], bfr, title)):
...
def keypress(event = None):
	...
	ln.redraw()
	for tag in type_.tag_names():
		if not tag in ('sel', 'marked', 'found', 'foundhighlight'): type_.tag_remove(tag, '1.0', 'end')
...
def f5(event = None):
	if filetype.cget('text') == 'Python File (*.py)':
		rp()
	elif filetype.cget('text') == 'LaTeX / TeX File (*.tex)':
		lp()
	elif filetype.cget('text') == 'HTML File (*.html)':
		hp()
	else:
...
def term():
	...
	if platform.system() == 'Linux':
		...
		proc = subprocess.Popen([shell], stdin = slave_fd, stdout = slave_fd, stderr = slave_fd, close_fds = True, preexec_fn = os.setsid, env = env)
		os.close(slave_fd)
		def _read():
			while running[0]:
				try:
		...
		proc = PtyProcess.spawn('powershell.exe', dimensions = (999999, 80))
		def _read():
			while running[0]:
				try:
					data = proc.read(4096)
...
def cmdrun(command):
	...
	if command in pcwrittencommands.keys():
		...
		try:
			...
			type_.tag_config('marked', background = 'light grey')
		...
		if not filetype.cget('text') in ('Python File (*.py)', 'LaTeX / TeX File (*.tex)', 'HTML File (*.html)'):
...
def pcread():
	...
	pycodecommands = ['aboutpynotes', 'ask', 'cleareditor', 'close', 'cmdrun', 'commentregion', 'commentselection', 'copy', 'copytext', 'cut', 'delete', 'dictate', 'downloadplugins', 'findreplace', 'findtext', 'fullscreen', 'getselection', 'gotoline', 'hmode', 'indentregion', 'indentselection', 'insert', 'killquit', 'mark', 'markselection', 'mathgod', 'maximize', 'minimize', 'movecursor', 'newfile', 'openfile', 'openhelp', 'openplugindir', 'openpycode', 'openterm', 'pageback', 'pageforw', 'paste', 'preferences', 'pynotessourcecode', 'pyshell', 'redo', 'repeatxcommand', 'runcode', 'savefile', 'say', 'selall', 'select', 'show', 'speaktext', 'switcheditor', 'termexec', 'tkindex', 'typecommand', 'uncommentregion', 'uncommentselection', 'undo', 'unfullscreen', 'unmark', 'unmarkall', 'wait', 'wintitle', 'write']
	pythoncommands = ['abt()', 'pcask', 'pccleareditor()', 'ext()', 'cmdrun', 'pccommentregion', 'pccommentselection()', 'cp()', 'pccopytext', 'cut()', 'type_.delete', 'st()', 'dp()', 'fr()', 'f()', 'pcfullscreen()', 'pcgetselection()', 'gl', 'pchmode', 'pcindentregion', 'pcindentselection()', 'type_.insert', 'exit()', 'pcmark', 'pcmarkselection()', 'mathgod()', 'pcmax()', 'root.iconify()', 'pcmovecursor', 'nw()', 'llld()', 'pcopenhelp', 'op()', 'pc()', 'term()', 'ptb()', 'ptf()', 'pst()', 'prf()', 'ss()', 'pcpyshell()', 'redo()', 'pcrepeatx', 'f5()', 'sssv()', 'say', 'selall()', 'pcselecttext', 'show', 'spk()', 'pcswitchedit()', 'pctermexec', 'pctkindex', 'cmd()', 'pcuncommentregion', 'pcuncommentselection()', 'undo()', 'pcunfullscreen()', 'pcunmark', 'pcunmarkall()', 'time.sleep', 'root.title', 'pccmdwrite']
	pcwrittencommands = {}
		...
		if line_:
			try:
				...
				if ks and len(ks) == 1:
					...
					cdt += f"type_.bind('{ks.split('→')[0].strip().strip()}', lambda event: exec(\"{'\\n'.join(map(lambda string: pycodeindex(string.strip()) if string else '', ks.split('→')[1].strip()[:-1][1:].strip().split('↩')))}\") or 'break')" + '\n'
					cdt += f"root.bind('{ks.split('→')[0].strip().strip()}', lambda event: exec(\"{  '\\n'.join(map(lambda string: pycodeindex(string.strip()) if string else '', ks.split('→')[1].strip()[:-1][1:].strip().split('↩')))}\"))" + '\n'
				elif v and len(v) == 1:
					v = v[0].strip()[:-1]
...
def helppycode():
	...
	hpwin.text(master = kst, text = '<Control-t> → <say \'Hello!\' ↩ say \'PyNotes is the best!\'>;', style = 'CodeStyle.TLabel').grid(column = 0, row = 2, padx = 10, pady = 10, sticky = 'w')
...
type_ = root.textbox(master = mf, undo = True, font = defs[2], wrap = 'word')
...
fm.add_command(label = 'Open → Ctrl + L / Alt + X - o', command = llld)
...
def hpvps():
	...
	for var in definedvars:
		...
		while True:
			...
			n = nn
	exec("shellcmd.tag_config('hpv'," + theme['python:variable_names'] + ')')
def hpfps():
	definedfuncs = []
	...
	for var in definedfuncs:
		...
		while True:
			...
			n = nn
	exec("shellcmd.tag_config('hpf'," + theme['python:function_names'] + ')')
def hpops():
	n = '1.0'
	searchstr = r'==|\!=|<|>|<=|>='
...
def hapyshell():
	...
	for i in range(1, lines + 1):
		prefix = shellcmd.get(f'{i}.0', f'{i}.{lenprompt}')
		if prefix in {'>>> ', '... '}:
			for tag in syntax_tags:
				shellcmd.tag_remove(tag, f'{i}.0', f'{i}.{lenprompt}')
		else:
			for tag in syntax_tags:
...
def shellpy():
	...
	def _start():
		...
		if platform.system() == 'Linux':
			...
			fcntl.ioctl(master_fd, termios.TIOCSWINSZ, struct.pack('HHHH', 24, 80, 0, 0))
			env['TERM'] = 'linux'
			proc = subprocess.Popen([sys.executable, '-u', '-i'], stdin = slave_fd, stdout = slave_fd, stderr = slave_fd, close_fds = True, preexec_fn = os.setsid, env = env)
			os.close(slave_fd)
			_proc_ref[0] = proc
			...
			proc = PtyProcess.spawn(sys.executable + ' -u -i', dimensions = (999999, 80))
			_proc_ref[0] = proc
			def _read():
				while running[0] and gen == generation[0]:
					try:
