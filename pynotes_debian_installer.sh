...
url_pynotes="https://github.com/rafugafu/PyNotes/releases/download/v$version/pynotes-$version-1.deb"
if ! wget -O "pynotes-$version-1.deb" "$url_pynotes"; then
  printf "\033[31merror: could not download PyNotes version '%s', check that it exists and is correct.\033[0m\n" "$version"
  rm -f "pynotes-$version-1.deb"
  exit 1
fi
apt install --reinstall --allow-downgrades "./pynotes-$version-1.deb"
rm -f "pynotes-$version-1.deb"
