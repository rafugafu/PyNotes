	...
	try:
		for iterator in range(len(code.split('\n'))):
			...
			if func:
				...
				exec(func[0].split('func ', 1)[1].split(',', 2)[0].strip() + '=lambda ' + func[0].split('func ', 1)[1].split(',', 2)[1].strip().replace(' ', ',') + ':' + funcfunc, globals())
				translatedline = f'print("{sympify(funcfunc)}")'
			if eq:
				eq = refind.findall(r'\{[ \t]*eq[ \t]+[a-zA-Z0-9_]+[ \t]*,[^}]*\}', translatedline)
				eqname = eq[0].split('eq ', 1)[1].split(',', 1)[0].strip()
				exec(eqname + '=Eq(' + eq[0].split('eq ', 1)[1].split(',', 1)[1].strip()[:-1].replace('=', ',') + ')', globals())
				translatedline = f'print("{eqname}")'
			if plot:
				...
				try:
					...
					translatedline = f'print("{plot[0].split('plot ')[1].strip()[:-1].split(',')[0].strip()}")'
			if plotcoord:
			...
			if piechart:
				...
				translatedline = f'print("{piechart[0].split('pie ')[1].strip()[:-1]}")'
			if barchart:
				barchart = refind.findall(r'\{[ \t]*bar[ \t]+[^}]*\}', line)
...
def retranslateandprint(string):
	...
	return 'break'
def ha():
	searchs = [r'\yplotlist\y', r'\yeval\y', r'\yRisingFactorial\y', r'\yFallingFactorial\y', r'\ynot\y', r'\yor\y', r'\yand\y', r'\yif\y', r'\yelse\y', r'\yabs\y', r'\yround\y', r'\ybar\y', r'\ypie\y', r'\yTrue\y', r'\yFalse\y', r'\yceiling\y', r'\yfloor\y', r'\ygamma\y', r'\ysolve\y', r'\ysqrt\y', r'\ysin\y', r'\ycos\y', r'\ytan\y', r'\ysum\y', r'\yprod\y', r'\yintegrate\y', r'\yderivative\y', r'\ylimit\y', r'\yplot\y', r'\yplot3\y', r'\yinterpolate\y', r'\yfunc\y', r'\yeq\y', r'\'.+?\'', r'".+?"', r'\d+', r'\yclearplot\y', r'\yPiecewise\y', r'\^.', r'_.', r'\^\(.+?\)', r'_\(.+?\)']
	tags = ['plotlist', 'eval', 'ffact', 'rfact', 'not', 'or', 'and', 'if', 'else', 'abs', 'rnd', 'bar', 'pi', 'tr', 'fa', 'cel', 'flr', 'gma', 'slve', 'sqrt', 'sin', 'cos', 'tan', 'sum', 'prod', 'int', 'der', 'lim', 'pl', 'pl3', 'inte', 'func', 'eq', 'quos', 'quod', 'num', 'cp', 'ifother', 'power', 'subscript', 'power_2', 'subscript_2']
	effects = ['bold', 'orange', 'bold', 'bold', 'orange', 'orange', 'orange', 'bold', 'bold', 'orange', 'orange', 'bold', 'bold', 'red', 'red', 'orange', 'orange', 'orange', 'bold', 'orange', 'orange', 'orange', 'orange', 'orange', 'orange', 'bold', 'bold', 'bold', 'bold', 'bold', 'bold', 'orange', 'orange', 'green', 'green', 'blue', 'bold', 'orange', 'up', 'down', 'up', 'down']
