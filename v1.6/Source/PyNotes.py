...
def changes(event = None):
	...
	cw.text(master = cw_, text = '1. Added Plugins to PyNotes!\n(Download by manually going to\nhttps://github.com/rafugafu/PyNotes/tree/main/Plugins\nor Plugins → Download From PyNotes\' GitHub)\nMade the chess game, letter invaders game, and typing a plugin and not part of PyNotes.\n\nNote: New Plugins can be added without updates to PyNotes, and will not be mentioned in updates\nto PyNotes.').grid(column = 0, row = 0, padx = 10, pady = 10, sticky = 'w')
	cw.text(master = cw_, text = '2. Fixed a bug where PyCode would sometimes not run on Windows.').grid(column = 0, row = 1, padx = 10, pady = 10, sticky = 'w')
	cw.text(master = cw_, text = '3. Made the syntax highlighting much better and faster and more efficient for large files.').grid(column = 0, row = 2, padx = 10, pady = 10, sticky = 'w')
	cw.text(master = cw_, text = '4. Made the regular backups much better.').grid(column = 0, row = 3, padx = 10, pady = 10, sticky = 'w')
	cw.text(master = cw_, text = '5. Added a menu for PyCode.').grid(column = 0, row = 4, padx = 10, pady = 10, sticky = 'w')
...
def hemail(event = None):
	...
	hew.text(master = dt, text = 'The email textbox has a spellcheck. The default dictionary for spellchecking is just english, but you can add or remove extra dictionaries to this.\nTo add another language or dictionary, you need to find or make a text file that has one word in each line, without any spaces. (It can be any language)\nThen go to Preferences → Email to upload that dictionary.\nONCE YOU HAVE ADDED A DICTIONARY, DO NOT MOVE THE FILE BEFORE REMOVING THE DICTIONARY IN THE PREFERENCES.').grid(padx = 10, pady = 10)
	hew.sizablefalse()
...
def prf(event = None):
	...
	pr.check(master = gt, text = 'Backup open file regularly', command = lambda: bf(bfc.get()), var = bfc).grid(column = 0, row = 0, sticky = 'w')
	varts = pr.booleanvar(value = taborspace)
	pr.check(master = gt, text = "Python: Use spaces instead of tabs for the 'ir' or 'indent-region' command", command = lambda: setts(varts.get()), var = varts).grid(column = 0, row = 1, sticky = 'w')
...
def cmdrun(command):
	...
	if command in plgncmds.keys():
		try:
			...
			root.error('Error!', f'There was an error in running the command {command}\nfrom the plugin "{os.path.basename(os.path.normpath(plgncmds[command][0]))}":\n{error}')
		...
		st()
	elif command == 'hap' or command == 'helpaddplugins':
		ap()
...
def pcread():
	global root
	pycodecommands = ['mathgod', 'wait', 'quit', 'say', 'command', 'selall', 'copy', 'paste', 'ask', 'cut', 'term', 'cmd', 'undo', 'redo', 'termexec']
	pythoncommands = ['mathgod()', 'time.sleep', 'root.destroy()', 'say', 'cmd()', 'selall()', 'cp()', 'pst()', 'ask', 'cut()', 'term()', 'cmdrun', 'undo()', 'redo()', 'termexec']
...
def pc(event = None):
	...
	commands = ['mathgod', 'wait', 'quit', 'say', 'command', 'selall', 'copy', 'paste', 'ask', 'cut', 'term', 'cmd', 'undo', 'redo', 'termexec', 'Done']
	buttons = []
	commandtodo = []
	commanddone = []
