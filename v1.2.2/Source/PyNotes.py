...
def changes(event = None):
    ...
    cw.text(master = cw_, text = '1. Added a shell for Python').grid(column = 0, row = 0, sticky = 'w')
    cw.text(master = cw_, text = '2. Fixed the bug where changing a shorcut in PyCode just added a new one\n instead of changing the old one').grid(column = 0, row = 1, sticky = 'w')
    cw.text(master = cw_, text = '3. Added line numbering in the editor').grid(column = 0, row = 2, sticky = 'w')
...
tabs.pack(padx = 10, pady = 10, fill = 'both')
scrlbr = root.scroll(master = mf)
scrlbr.pack(side = 'right', fill = 'y')
ln = root.textbox(master = mf, width = 3, state = 'disabled', font = defs[2], yscrollcommand = lambda *args: ln.yview_moveto(type_.yview()[0]))
...
shellcmd = root.textbox(master = sf, height = 1)
shellcmd.grid(column = 1, row = 0, padx = 5, pady = 5)
shellout = root.listbox(master = sf, width = 80)
shellout.grid(column = 1, row = 1, padx = 5, pady = 5)
root.button(master = sf, text = 'Exec', command = lambda: [shellout.insert('end', line) for line in shellexec(shellcmd.get('1.0', 'end')).split('\n')] + [shellcmd.delete('1.0', 'end'), shellout.insert('end', '\n'), shellout.see('end')]).grid(column = 0, row = 1, padx = 5, pady = 5)
root.bind('<Alt-x>', cmd)
