...
try:
	...
	truncate = root.ask('Warning', 'The way preferences are stored in this version of PyNotes is changed.\nYou have been using an older version of PyNotes\nShould PyNotes automatically delete those old preferences?', ('yes', 'no'))
	if not truncate:
...
def changes(event = None):
	...
	cw.text(master = cw_, text = '1. Added html support to PyNotes! (Alt-X → hmode:html to change the HMode)').grid(column = 0, row = 0, sticky = 'w')
...
def htags():
	type_.tag_remove('htags', '1.0', 'end')
	...
	for tag in tags:
		...
		while True:
			...
			if not n: break
			nn = '%s+%dc' % (n, count.get())
			type_.tag_add('htags', n, nn)
			n = nn
	type_.tag_config('htags', foreground = 'red')
def hattr():
	...
	for tag in tags:
		...
		while True:
			...
			n = nn
	type_.tag_config('hattr', foreground = 'magenta')
def hstuff():
	...
	while True:
		...
		n = nn
	type_.tag_config('hstuff', foreground = 'orange')
def hcmt():
	...
	while True:
		...
		n = nn
	type_.tag_config('hcmt', foreground = 'grey')
def hstr():
	...
	while True:
		...
		type_.tag_add('hstr', n, nn)
		n = nn
	type_.tag_config('hstr', foreground = 'green')
def ha(ft):
	if ft == 'py':
		hpa()
		hpb()
