...
class win(tk.Tk):
    def __init__(self, style = True, *args, **kwargs):
        ...
        self.tk.call('source', os.path.join(self.path, 'acid/acid.tcl'))
        self.tk.call('source', os.path.join(self.path, 'fire/fire.tcl'))
        self.tk.call('source', os.path.join(self.path, 'ice/ice.tcl'))
        if type(style) == str:
    ...
    def askstring(self, title, prompt):
        ...
        root.button(text = 'Cancel', master = bf, command = root.destroy).pack(side = 'left', padx = 5, pady = 5)
    ...
    def listbox(self, master = None, *args, **kwargs):
        if master == None:
            master = self
        return tk.Listbox(master, *args, **kwargs)
    ...
    def openfile(self, types = ['all', 'py']):
    ...
    def save(self):
        if self.file.get()[len(self.file.get()) - 1] == '/':
            ...
            if os.path.exists(os.path.join(os.getcwd(), self.file.get())):
                if self.root.ask('WARNING', 'A file named "{}" already exists. Do you want to replace it?'.format(self.file.get()), ('yes', 'no')):
