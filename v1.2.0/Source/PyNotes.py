...
def changes(event = None):
    ...
    style = cw.style()
    style.configure('Background.TLabel', background = 'lightblue')
    cw.text(master = cw_, text = '1. Added PyCode (Elisp)!', padding = (5, 5, 5, 5), relief = 'raised', style = 'Background.TLabel').grid(column = 0, row = 0, sticky = 'w')
    cw.text(master = cw_, text = '2. Fixed a few important bugs').grid(column = 0, row = 1, sticky = 'w')
    cw.text(master = cw_, text = '3. Added a few more commands').grid(column = 0, row = 2, sticky = 'w')
...
def cmdrun(command):
    ...
    if command == 'exit' or command == 'e':
        ...
        ans = root.askstring('Change hmode', 'HMode (python / latex / normal):').lower().replace(' ', '')
...
def pcread():
    ...
    for line in code:
        ...
        if line:
            ...
            if ks and len(ks) == 1:
                ks = ks[0]
                pcrun(f"root.bind('{ks.split('→')[0]}', lambda event: pcrun(\"{ks.split('→')[1][:-1]}\"))")
...
type_ = root.textbox(master = mf, yscrollcommand = scrlbr.set, font = defs[2], wrap = 'word')
...
filename = root.text(master = fileinfo, text = '*scratch*', padding = (5, 5, 5, 5), relief = 'raised')
...
root.bind('<Alt-l>', gl)
om.add_command(label = 'Page turn → Ctrl + P / Alt + X - p', command = ptf)
...
hm.add_separator()
hm.add_command(label = 'Help with PyCode', command = lambda: root.info('Help with PyCode', 'This is still very simple. You can only define new keyboard shortcuts till now.\nYou also need to know a bit about python and tkinter. Example:\nIf you want to define Control + Q to quit the window, you can type this:\n\n<Control-q> → root.destroy();\n\nLike this, Alt is <Alt>, and so on. This just uses the root.bind function, so if you know about tkinter, you can use all those things.\nAnything that is not like\n\n<...> → command;\n\nwill be ignored as a comment.\nRemember you cannot redefine default shortcuts or keys\nAny errors will be displayed immediately after you have finished editing.'))
pcread()
if len(sys.argv) > 1:
    ld(sys.argv[1])
root.show()
