...
FORT_SECTIONS = 80
WORDS = open('/usr/share/PyNotes/wordlist.txt', 'r').read().split('\n')
...
class Game:
	def __init__(self):
		...
		self.score = 0
		try:
			os.mkdir(f'/home/{getpass.getuser()}/.local/PyNotes/game')
		except:
			pass
		try:
			self.highscore = int(open(f'/home/{getpass.getuser()}/.local/PyNotes/game/highscore.txt', 'r').read().replace('\n', ''))
		except:
			self.highscore = 0
			open(f'/home/{getpass.getuser()}/.local/PyNotes/game/highscore.txt', 'w')
		self.round = 0
	...
	def game_over(self):
		...
		open(f'/home/{getpass.getuser()}/.local/PyNotes/game/highscore.txt', 'w').write(str(self.highscore))
