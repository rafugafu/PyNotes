...
def changes(event = None):
	...
	cw.text(master = cw_, text = '1. Added syntax highlighting to the Python shell!').grid(column = 0, row = 0, sticky = 'w')
	cw.text(master = cw_, text = '2. The preferences menu is now opened the first time PyNotes is run.').grid(column = 0, row = 1, sticky = 'w')
	cw.text(master = cw_, text = '3. Added a button to clear the Python shell.').grid(column = 0, row = 2, sticky = 'w')
...
def hapyshell():
	...
	for i in range(2, lines):
		for tag in shellcmd.tag_names():
			if tag == 'prompt':
				continue
			if shellcmd.get(f'{i}.0', f'{i}.{lenprompt}') in {'>>> ', '... '}:
