...
class win(ttk.Window):
	...
	def ask(self, title, question, options, *args, **kwargs):
		...
		if options == ('yes', 'no'):
			return {'Yes': True, 'No': False, None: None}[mb.yesno(title = title, message = question, *args, **kwargs)]
		if options == ('ok', 'cancel'):
			return {'OK': True, 'Cancel': False, None: None}[mb.okcancel(title = title, message = question, *args, **kwargs)]
		if options == ('yes', 'no', 'cancel'):
			return {'Yes': True, 'No': False, 'Cancel': None, None: None}[mb.yesnocancel(title = title, message = question, *args, **kwargs)]
		if options == ('retry', 'cancel'):
			return {'Retry': True, 'Cancel': False, None: None}[mb.retrycancel(title = title, message = question, *args, **kwargs)]
	def strok(self, ans):
		self.strans = ans
