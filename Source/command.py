"""The Alt-X command line: parsing/splitting command syntax (';' to
chain commands, ':(...)' to group input containing literal semicolons,
'\\' to escape a following character), the giant command dispatch table
in cmdrun(), and the separate cmdregistry used purely to drive Alt-X's
autocomplete (see cmdautocompletefunc)."""
...
def cmdfindgroupclose(s, openindex):
    """Return the index in s of the ')' matching the '(' at openindex
    (honoring nesting and backslash-escaped characters), or None if
    unmatched."""
    depth = 0
    i = openindex
    n = len(s)
...
def cmdcheckbrackets(s):
    """Whether every ':(' group in s (a command's input wrapped in
    parens, e.g. 're:(...)*3') has a matching closing ')'."""
    i = 0
    n = len(s)
    while i < n:
        ch = s[i]
...
def cmdstripgroup(commandinput):
    """If commandinput is entirely wrapped in one matching pair of
    parens, strip them (letting a command's input itself start with '('
    without being mistaken for a ':(...)' group by other commands)."""
...
def cmdsplit(s):
    """Split s on top-level ';' into separate chained commands, without
    splitting on ';' inside a ':(...)' group or after a backslash
    escape."""
    parts = []
    current = ""
    i = 0
...
def cmdparsegroup(s):
    """Parse the 'content*n' or '(content)*n' repeat syntax used by the
    write/w and repeat/re commands; return (content, n). Raises if s
...
def cmdrun(fullcommand):
    """Run one Alt-X command line: split chained ';'-separated commands
    and run each in turn; otherwise split fullcommand into a command
    name and (optionally ':(...)'-grouped) input, run plugin-defined
    commands (state.plgncmds) or PyCode-written commands
    (state.pcwrittencommands) if the name matches one, and otherwise
    dispatch to PyNotes' own built-in command implemented below (most of
    which simply reject any input before acting, since they take none).
    Runs the before-hook unconditionally, but the after-hook only if the
    matched branch doesn't return early (e.g. on invalid input or an
    unmet precondition).
    """
    if not fullcommand:
        return
    ...
    if command in state.plgncmds:
        try:
            ...
            state.root.error(
                "Error!",
                f"There was an error in running the command '{command}' from the plugin \"{os.path.basename(os.path.normpath(state.plgncmds[command][0]))}\":\n{error}",
            )
    ...
    if command in state.pcwrittencommands:
        try:
            ...
            state.root.error(
                "Error!",
                f"There was an error in running the command '{command}' defined in PyCode:\n{error}",
            )
        ...
        command == "sh"
        or command == "splithoriz"
        or command == "split-editor-horizontal"
    ):
        if commandinput:
            utils.show(f"error: command '{command}' does not take input")
            return
        ...
        command == "sv" or command == "splitvert" or command == "split-editor-vertical"
    ):
        if commandinput:
            utils.show(f"error: command '{command}' does not take input")
            return
        ...
        command == "setsel"
        or command == "selpointset"
        or command == "selection-point-set"
    ):
        if not isinstance(state.active, editor.Editor):
            utils.show("not an editor")
            return
        ...
        command == "unsetsel"
        or command == "selpointunset"
        or command == "selection-point-remove"
    ):
        if not isinstance(state.active, editor.Editor):
            utils.show("not an editor")
            return
        ...
        command == "cb"
        or command == "close"
        or command == "closecurbuf"
        or command == "close-current-buffer"
    ):
        if commandinput:
            try:
                n = int(commandinput)
                ...
                utils.show(
                    f"error: invalid input '{commandinput}' to close editor command"
                )
                return
        ...
        command == "sw"
        or command == "switch"
        or command == "switchbuf"
        or command == "switch-buffer"
    ):
        if commandinput:
            utils.show(f"error: command '{command}' does not take input")
            return
        ...
        command == "neh"
        or command == "newedithoriz"
        or command == "new-editor-horizontal"
    ):
        if commandinput:
            utils.show(f"error: command '{command}' does not take input")
            return
        ...
        command == "nev" or command == "neweditvert" or command == "new-editor-vertical"
    ):
        if commandinput:
            utils.show(f"error: command '{command}' does not take input")
            return
        ...
        command == "onh"
        or command == "opennewhoriz"
        or command == "open-file-horizontal"
    ):
        if commandinput:
            utils.show(f"error: command '{command}' does not take input")
            return
        ...
        command == "onv" or command == "opennewvert" or command == "open-file-vertical"
    ):
        if commandinput:
            utils.show(f"error: command '{command}' does not take input")
            return
        ...
        if commandinput and commandinput not in ("h", "horiz", "horizontal"):
        ...
        try:
            ...
            state.active.type_.insert(
                state.active.type_.index("insert"),
                textwrote.encode().decode("unicode_escape") * timeswrote,
            )
        ...
        if commandinput and commandinput not in ("b", "back"):
        ...
        if commandinput and commandinput not in ("b", "back"):
            utils.show(
                f"error: invalid input '{commandinput}' for find & replace command"
            )
            return
        if not isinstance(state.active, editor.Editor):
            utils.show("not an editor")
        ...
        command == "terminal" or command == "cmd" or command == "term" or command == "t"
    ):
        if commandinput:
            ...
            terminal.term(
                command=commandlist,
                endmessage="--- Command finished, press any key to continue ---",
            )
        else:
            terminal.term()
        ...
        getattr(state.active, "hmode", False) not in ["png", "pdf", "epub"]
        and command == "hmode"
    ):
        if not commandinput:
            utils.show(f"error: no input given to command '{command}'")
        ...
        if not (
            ...
            or commandinput in state.plgnhmodes
        ):
        ...
        command == "pynavstart"
        or command == "pyjumpstart"
        or command == "python-jump-startof"
    ):
        if not commandinput:
            utils.show(f"error: no input given to command '{command}'")
            return
        ...
        command == "pynavend"
        or command == "pyjumpend"
        or command == "python-jump-endof"
    ):
        if not commandinput:
            utils.show(f"error: no input given to command '{command}'")
            return
...
def cmdallhmodenames():
    """Every valid HMode name, built-in plus plugin-provided."""
...
def cmdrevalues(currentinput):
    """Autocomplete values for repeat/re's input: whatever commands
    could follow at the current point inside its '(...)*n' or 'command*n'
    syntax."""
    if currentinput.startswith("("):
        closeindex = cmdfindgroupclose(currentinput, 0)
        if closeindex is None:
            return [
                "(" + candidate for candidate in cmdautocompletefunc(currentinput[1:])
            ]
        return []
    if "*" in currentinput:
        content, multiplier = currentinput.split("*", 1)
        return [
            candidate + "*" + multiplier for candidate in cmdautocompletefunc(content)
        ]
    return cmdautocompletefunc(currentinput)


def cmdregister(names, hmodes=None, inputs=None, buffertypes=None):
    """Register every alias in names (all naming the same command) in
    cmdregistry, recording when it should be offered/how its input
    should autocomplete: hmodes restricts it to editors in one of those
    HModes (or is a callable returning the current list); inputs is a
    fixed list of valid input values (None allowed in the list to also
    offer the bare, no-input form) or a callable(currentinput) ->
    candidates; buffertypes restricts it to those buffer classes.
    Leaving an argument None means "no restriction"/"free-form input".
    This registry only drives autocomplete (cmdautocompletefunc) - it
    plays no part in actually running a command, which cmdrun() (above)
    does via its own separate dispatch chain.
    """
    for name in names:
        cmdregistry[name] = {
            "hmodes": hmodes,
...
cmdregister(
    ("bb", "balance", "balance-buffers"),
    inputs=[None, "h", "horiz", "horizontal", "v", "vert", "vertical"],
)
...
cmdregister(
    ("comment", "cr", "comment-region"),
    hmodes=("python", "latex", "html", "markdown"),
    buffertypes=(editor.Editor,),
)
cmdregister(
    ("uncomment", "uncr", "uncomment-region"),
    hmodes=("python", "latex", "html", "markdown"),
    buffertypes=(editor.Editor,),
)
...
cmdregister(
    ("pynavstart", "pyjumpstart", "python-jump-startof"),
    hmodes=("python",),
    inputs=cmdpynavvalues,
    buffertypes=(editor.Editor,),
)
cmdregister(
    ("pynavend", "pyjumpend", "python-jump-endof"),
    hmodes=("python",),
    inputs=cmdpynavvalues,
    buffertypes=(editor.Editor,),
)
...
cmdregister(
    ("hmode",),
    hmodes=cmdallhmodenames,
    inputs=["python", "py", "latex", "la", "normal", "norm", "html", "markdown", "md"]
    + list(state.plgnhmodes),
    buffertypes=(editor.Editor,),
)


def cmdregistryentry(name):
    """cmdregistry[name], or the unrestricted default if name isn't
    registered (e.g. a plugin or PyCode-written command)."""
    return cmdregistry.get(name, {"hmodes": None, "inputs": None, "buffertypes": None})


def cmdbuffertypeavailable(buffertypes):
    """Whether the active buffer is one of buffertypes (or buffertypes
    is None, meaning any buffer)."""
...
def cmdhmodeavailable(hmodes):
    """Whether the active buffer is an Editor currently in one of
    hmodes (or hmodes is None, meaning any/no HMode restriction)."""
    if hmodes is None:
        return True
    if callable(hmodes):
        hmodes = hmodes()
...
def cmdinputvariants(inputs):
    """From a registry entry's inputs, return (show_bare, show_colon):
    whether the command should be offered with no input (bare) and/or
    with a trailing ':' (input to follow)."""
    if inputs is None:
        return True, False
...
def cmdbasecommandnames():
    """Every command name (bare, and/or with a trailing ':') worth
    offering as an autocomplete candidate right now, given the active
    buffer's type and HMode."""
...
def cmdcommandvalues(command, currentinput):
    """Autocomplete candidates for command's input, given what's typed
    of it so far (currentinput); [] if command takes no input or isn't
    currently available."""
...
def cmdactivesegment(typedtext):
    """Split typedtext into (prefix, active segment): prefix is
    everything up to and including the last completed ';'-separated
    command (plus any leading whitespace of the current one), and the
    active segment is the command currently being typed/completed."""
    prefix = ""
    while True:
...
def cmdautocompletefunc(typedtext):
    """The Alt-X prompt's autocomplete function: candidates for the
    command name if the active segment has no ':' yet, or for that
    command's input if it does, each returned as a full replacement for
    typedtext (prefix included)."""
    ...
    if ":" in activesegment:
        ...
        if not values:
            return []
        return [
            prefix + commandpart + ":" + valueprefix + value
            for value in sorted(values)
            if value.startswith(valuestripped)
        ]
    candidates = [
        name for name in cmdbasecommandnames() if name.startswith(activesegment)
    ]
