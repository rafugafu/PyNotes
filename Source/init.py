"""Early startup: platform paths, first-run bootstrapping (venv,
optional dependency installation), and building the root window, its
menus, and loading/validating saved preferences."""
...
def info(title, inf):
    """Show a standalone info popup with a Close button, blocking (its
    own mainloop) until closed. Used before state.root exists yet."""
    import easytk

    infowin = easytk.win()
    infowin.title(title)
    infowin.text(text=inf).pack(side="top", anchor="n", padx=10, pady=10)
    infowin.button(text="Close", command=infowin.destroy).pack(
        side="right", anchor="se", padx=10, pady=10
    )
    infowin.show()


def ask(title, askinput):
    """Show a standalone Yes/No popup, blocking (its own mainloop) until
    answered, and return the answer as a bool. Used before state.root
    exists yet (e.g. to ask about installing a missing dependency)."""
    import easytk
    ...
    askwin.button(text="Yes", command=yes).pack(
        side="left", anchor="nw", padx=10, pady=10
    )
    askwin.button(text="No", command=no).pack(
        side="right", anchor="ne", padx=10, pady=10
    )
    # Disable the window's close button so Yes/No is the only way out.
...
def switchvenv():
    """On Linux, create PyNotes' own virtual environment on first run,
    then re-exec the current process under that venv's Python if not
    already running from it (so dependencies installed via
    ensure_dependencies() stay isolated from the system Python)."""
    if platform.system() != "Linux":
        return
    ...
    if not os.path.exists(f"{venvdir}/bin/python"):
        mb.showinfo(
            ...
            "Making the Python virtual environment for PyNotes. This only happens on the first startup of a new\ninstallation of PyNotes.",
        )
...
def ensure_dependencies():
    """Check every optional third-party dependency PyNotes needs; for
    each missing one, ask to pip-install it (locally, via
    dialogs.faketerm) or quit. Finishes by re-importing everything to
    confirm the installs actually succeeded, quitting if not."""
    import dialogs
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'tika' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'pdfplumber' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'pyttsx3' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'matplotlib' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'sympy' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'sounddevice' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'speech_recognition' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'numpy' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'tklinenums' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'ziamath' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'cairosvg' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'Pillow' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    try:
        ...
        ans = ask(
            "Error!",
            "The module 'watchdog' is not installed. Should PyNotes install it locally?",
        )
        if not ans:
            exit()
        else:
    ...
    if platform.system() != "Linux":
        try:
            ...
            ans = ask(
                "Error!",
                "The module 'pywinpty' is not installed. Should PyNotes install it locally?",
            )
            if not ans:
                exit()
            else:
...
def load_or_create_defs():
    """Open the saved preferences (defs) file, creating it (from
    defaultdefs, the built-in default preferences/theme) with a welcome
    message on first run. Returns (open file handle positioned at the
    start, whether it was just created, defaultdefs)."""
...
def load_plugins(no_load_plugins):
    """Discover every plugin in the add-ons directory (unless
    no_load_plugins) and load its files: Alt-X commands, PyCode
    commands, HModes, their help text, and preferences code go straight
    into the matching state.plgn* globals; "init"/"first"/"last" code is
    instead returned (as lists of (plugin_dir, code) pairs) for the
    caller to exec at three different points of startup - "init" before
    buffer/editor classes are even imported (so it can populate
    state.buffer_init_functions etc. for their class bodies to pick up),
    "first" once the root window exists, and "last" once startup is
    otherwise complete."""
    init = []
    first = []
    last = []
    ...
    plgns = [
        os.path.join(f"{homedir}/.local/share/PyNotes/add-ons", plgn)
        for plgn in plgns
        if os.path.isdir(os.path.join(f"{homedir}/.local/share/PyNotes/add-ons", plgn))
    ]
    state.plgncmds = dict()
    init = []
    first = []
    ...
    # commands/hmodes/pycodecommands files hold one entry per line, each
    # formatted as "name"code (a quoted name immediately followed by
    # that command/HMode's Python code), e.g. "spellcheck"pcrun(...).
    for plgn in plgns:
        ...
        try:
            ...
            try:
                ...
                info(
                    "Error!",
                    f'There was an error in loading the HModes of the plugin "{os.path.basename(os.path.normpath(plgn))}":\n{error}',
                )
                exit()
        try:
            ...
            try:
                ...
                info(
                    "Error!",
                    f'There was an error in loading the PyCode commands of the plugin "{os.path.basename(os.path.normpath(plgn))}":\n{error}',
                )
                exit()
        try:
        ...
        try:
            ...
            try:
                ...
                info(
                    "Error!",
                    f'There was an error in loading the Alt-X commands of the plugin "{os.path.basename(os.path.normpath(plgn))}":\n{error}',
                )
                exit()
...
def create_root_and_menus():
    ...
    state.all_editor_menus = {
        "File": state.fm,
        "Edit": state.em,
        "HMode": state.hmm,
        **state.all_buffer_menus,
    }
    state.all_terminal_menus = {
        "File": state.fm,
        "Edit": state.tem,
        **state.all_buffer_menus,
    }
    state.all_pythonshell_menus = {
        "File": state.fm,
        "Edit": state.tem,
        **state.all_buffer_menus,
    }
...
def load_config(file, defaultdefs):
    """Parse the defs file (opened by load_or_create_defs()) into
    state.defs/dicts/bfr/emacskeysforsearch/taborspace/
    nographicalfiledialogs/pythonexecutable/theme. If it's missing
    settings added since it was saved (an older-version defs file) and
    fails to parse, offer to reset it to defaultdefs; otherwise, if it's
    just from an older PyNotes version, notify the user and update its
    version line. Finishes by setting the window icon and initial size.
    """
    ...
    try:
        ...
        truncate = state.root.ask(
            "Warning",
            f"You are using preferences from an older version of PyNotes which do not have all the settings of this one.\nDo you want to reset the preferences and continue?\n(File: {homedir}/.local/share/PyNotes/defs)",
            ("yes", "no"),
        )
        if not truncate:
            ...
            state.root.destroy()
            exit()
        # Rewrite the defs file from defaultdefs and re-parse it the
        # same way as the try block above, minus the theme-key checks
        # (defaultdefs is always valid, so they'd be pointless here).
    ...
    try:
        ...
        state.root.error(
            "Error",
            f"Could not find the icon at {rootdir}/Icon.png.\nQuitting PyNotes.",
        )
        state.root.destroy()
        exit()
