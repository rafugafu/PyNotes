"""The Python Shell buffer: a real Python REPL running in an embedded
terminal (see terminal.Terminal), with the same scope-aware Python
syntax highlighting as the editor's Python HMode applied to its
transcript."""
...
class PythonShellBuffer(Buffer):
    """A buffer running a real `python` (or state.pythonexecutable)
    REPL process, with a "Clear Shell" / "Restart Shell" toolbar and
    live syntax highlighting of the REPL transcript. The
    _pyshell_session_* attributes carry name/type/scope information
    forward across statements already executed in this REPL session, so
    later input is highlighted using bindings made by earlier input,
    the same way a real Python session remembers them."""
    ...
    def __init__(self, master, *args, **kwargs):
        ...
        self.fileinfoconfig(
            buffertype="*Python Shell*", interpreter=state.pythonexecutable
        )
        self.hmode = "python"
        ...
        self._pyshell_session_inherited = {
            "members": set(),
            "attr_types": set(),
            "method_params": set(),
        }
    ...
    def init_pythonshell_hl_tags(self):
        """(Re-)configure every Python-shell syntax-highlight tag's
        appearance from the current theme (state.theme). The theme
        strings are Python attribute-config expressions written in
        terms of a `type_` placeholder, substituted here with
        `self.shellcmd` and exec'd to apply them."""
        exec(
            "self.shellcmd.tag_config('hpa',"
            + state.theme["python:keywords"].replace("type_", "self.shellcmd")
            + ")"
        )
        exec(
            "self.shellcmd.tag_config('hpb',"
            + state.theme["python:inbuilt"].replace("type_", "self.shellcmd")
            + ")"
        )
        exec(
            "self.shellcmd.tag_config('hpv',"
            + state.theme["python:variable_names"].replace("type_", "self.shellcmd")
            + ")"
        )
        exec(
            "self.shellcmd.tag_config('hpi',"
            + state.theme["python:class_instances"].replace("type_", "self.shellcmd")
            + ")"
        )
        exec(
            "self.shellcmd.tag_config('hpf',"
            + state.theme["python:function_names"].replace("type_", "self.shellcmd")
            + ")"
        )
        exec(
            "self.shellcmd.tag_config('hpx',"
            + state.theme["python:class_names"].replace("type_", "self.shellcmd")
            + ")"
        )
        exec(
            "self.shellcmd.tag_config('hpfa',"
            + state.theme["python:function_arguments"].replace("type_", "self.shellcmd")
            + ")"
        )
        exec(
            "self.shellcmd.tag_config('hpm',"
            + state.theme["python:module_names"].replace("type_", "self.shellcmd")
            + ")"
        )
        exec(
            "self.shellcmd.tag_config('hpo',"
            + state.theme["python:operators"].replace("type_", "self.shellcmd")
            + ")"
        )
        exec(
            "self.shellcmd.tag_config('hpd',"
            + state.theme["python:strings"].replace("type_", "self.shellcmd")
            + ")"
        )
        exec(
            "self.shellcmd.tag_config('hpc',"
            + state.theme["python:comments"].replace("type_", "self.shellcmd")
            + ")"
        )

    def shellpy(self):
        """Build the shell UI: the Clear/Restart Shell buttons and the
        embedded terminal running the Python interpreter, and start the
        periodic redraw that keeps the visible transcript highlighted."""
        lenprompt = len(">>> ")
        _hl_pending = [False]

        def colourprompts():
            """Bold-color every '>>> '/'... ' prompt prefix in the
            transcript so far."""
            lines = int(self.shellcmd.index("end-1c").split(".")[0])
            self.shellcmd.tag_remove("prompt", "1.0", "end")
            for i in range(1, lines + 1):
                if self.shellcmd.get(f"{i}.0", f"{i}.{lenprompt}") not in {
                    ">>> ",
                    "... ",
                }:
                    continue
                self.shellcmd.tag_add("prompt", f"{i}.0", f"{i}.{lenprompt}")
            self.shellcmd.tag_config(
                "prompt", foreground="green", font=(monospace, 12, "bold")
            )

        def _schedule_hl():
            """Coalesce bursts of terminal output into a single
            hapyshell() re-highlight, run once idle."""
        ...
        def _on_output(event):
            """Re-tag prompts and schedule a re-highlight after the
            terminal processes new output from the REPL process."""
            colourprompts()
            _schedule_hl()

        def _make_shellcmd():
            """Create the embedded terminal running the Python
            interpreter as its subprocess."""
            widget = terminal.Terminal(
                self, [state.pythonexecutable], None, nocolor=True
            )
        ...
        def cs():
            """Clear Shell: wipe the visible transcript and send a
            fresh prompt (Ctrl-L on Linux, or a plain Enter elsewhere)
            without restarting the underlying Python process, so the
            session's variables/imports are kept."""
            self._pyshell_last_scan_key = None
            self._pyshell_cached_scope_result = None
        ...
        def ks():
            """Restart Shell: kill and restart the underlying Python
            process, clearing all of this session's remembered
            variables/types/classes (the _pyshell_session_* state) along
            with it."""
            self._pyshell_last_scan_key = None
            self._pyshell_cached_scope_result = None
        ...
        clearshell = state.root.button(
            master=self._shellbuttons, text="Clear Shell", command=cs
        )
        killshell = state.root.button(
            master=self._shellbuttons, text="Restart Shell", command=ks
        )
        ...
        def shell_setview():
            """Re-highlight periodically (every 50ms) so highlighting
            keeps up as the process prints new output on its own, not
    ...
    def hapyshell(self):
        """Re-highlight the shell transcript; re-entrancy guard around
        _hapyshell_body() (which can trigger further tkinter events)."""
    ...
    def _hapyshell_body(self):
        """Re-highlight the visible part of the shell transcript.

        The transcript is a mix of REPL prompts ('>>> '/'... ') and
        their (possibly line-wrapped) Python source, plus the
        interpreter's own printed output. This strips the prompts back
        out into plain, "logical" Python source (stripped_lines) so it
        can be fed to python_scope_build's scope analysis exactly like
        an editor buffer would be, carrying forward this session's
        already-known names/types/classes as seed_* arguments so a
        later statement can reference bindings an earlier one made.
        Only the current viewport is actually re-tagged, for
        performance; _shell_pre_text is pre-scanned so a string/comment
        that started above the viewport is still recognized as such.
        widx()/off2lc() translate between offsets into the logical
        (prompt-stripped) source and real tkinter text indices, since
        those two coordinate spaces differ once lines wrap or continue.
        The scope-analysis result itself is cached (_pyshell_last_scan_key/
        _pyshell_cached_scope_result) since it doesn't change unless the
        logical source or its block structure actually changed.
        """
        lenprompt = len(">>> ")
        ...
        if _scan_key == self._pyshell_last_scan_key:
            ...
            shell_result = python_scope_build._python_build_scopes(
                self,
                stripped_text,
                line_blocks=_shell_line_blocks,
                seed_names=self._pyshell_session_names,
                seed_types=self._pyshell_session_types,
                seed_classes=self._pyshell_session_classes,
                seed_aliases=self._pyshell_session_aliases,
                seed_origins=self._pyshell_session_origins,
                seed_method_params=self._pyshell_session_method_params,
                seed_accepts_any=self._pyshell_session_accepts_any,
                seed_module_bases=self._pyshell_session_module_bases,
                seed_func_origins=self._pyshell_session_func_origins,
                seed_attr_types=self._pyshell_session_attr_types,
                seed_class_attr_types=self._pyshell_session_class_attr_types,
                seed_func_params=self._pyshell_session_func_params,
                seed_func_accepts_any=self._pyshell_session_func_accepts_any,
                seed_class_bases=self._pyshell_session_class_bases,
                seed_inherited=self._pyshell_session_inherited,
                seed_instance_only=self._pyshell_session_instance_only,
            )
        ...
        if shell_result is None:
            ...
            (
                ...
                shell_class_bases,
                shell_inherited,
                shell_module_scope_class_keys,
                shell_instance_only,
                shell_instance_name_positions,
                shell_global_stmt_kind_positions,
            ) = shell_result
        for _nm, _defs in shell_scopes[0]["names"].items():
            ...
            if (
                _exec_defs
                and _nm not in shell_scopes[0].get("globals", {})
                and _nm not in shell_scopes[0].get("nonlocals", {})
            ):
        ...
        for _cn, _mem in shell_local_classes.items():
            ...
            if (
                _cls_lines is None
                or _cn in self._pyshell_session_classes
                or any(_l < _exec_boundary for _l in _cls_lines)
            ):
        ...
        for _inhk in ("members", "attr_types", "method_params"):
            self._pyshell_session_inherited[_inhk].update(
                shell_inherited.get(_inhk, ())
            )
        ...
        try:
            ...
            def _removable(tag):
                return tag not in state._PYTHON_SHELL_HL_SKIP_REMOVE_TAGS and (
                    tag not in state.skiptagspythonshell
                    or self.hmode not in state.skiptagspythonshell[tag]
                )
            ...
            vis_code = [
                stripped_lines[L - 1] if 0 <= L - 1 < len(stripped_lines) else ""
                for L in vis_abs
            ]
            visible_code = "\n".join(vis_code)
            line_starts = []
            ...
            def widx(line, col):
                segs = (
                    _shell_seg_map[line - 1]
                    if 0 <= line - 1 < len(_shell_seg_map)
                    else None
                )
                if not segs:
                    _fr = (
                        _shell_logical_real_range[line - 1][0]
                        if 0 <= line - 1 < len(_shell_logical_real_range)
                        else line
                    )
                    return f"{_fr}.{col}"
                _cum = 0
            ...
            def _resolve_scope_idx(line, col):
                winner = None
                winner_start = None
                for _cstart, _ccol, _cend, _ecol, _ck in line_scope_candidates.get(
                    line, ()
                ):
            ...
            for lineno, dcol, dotted in shell_import_dotted_lines:
                shell_import_dotted_by_line.setdefault(lineno, []).append(
                    (dcol, dotted)
                )
            shell_import_orig_by_line = {}
            for _oln, _ocol, _oname, _otag in shell_import_orig_name_tags:
                shell_import_orig_by_line.setdefault(_oln, []).append(
                    (_ocol, _oname, _otag)
                )
            ...
            def _shell_same_block(l1, l2):
                if l1 == l2:
                    return True
                if not (0 < l1 <= len(_shell_line_blocks)) or not (
                    0 < l2 <= len(_shell_line_blocks)
                ):
                    return False
            ...
            for _nl, _ncol, _nname, _nstore in shell_name_positions:
                shell_name_pos_by_line.setdefault(_nl, []).append(
                    (_ncol, _nname, _nstore)
                )
            shell_def_names_by_line = {}
            for _dl, _dcol, _dname, _dkind in shell_def_names:
                shell_def_names_by_line.setdefault(_dl, []).append(
                    (_dcol, _dname, _dkind)
                )
            shell_kind_tags = {
                "var": "hpv",
            ...
            def _shell_active_for(abs_line, scope_idx):
                ...
                innermost_parent = (
                    shell_scopes[innermost_scope]["parent"]
                    if innermost_scope is not None
                    else None
                )
                on_header = (
                    innermost_scope is not None
                    and abs_line == shell_scopes[innermost_scope]["start"]
                )
                _redir_names = set()
                ...
                while _sidx is not None:
                    ...
                    for name, defs in sc["names"].items():
                        ...
                        if (
                            ...
                            and _shell_same_block(latest[0], abs_line)
                            and name not in _PYTHON_BUILTIN_NAMES
                        ):
                            best = latest
                        bound.add(name)
                        if best is not None:
            ...
            for li, abs_line in enumerate(vis_abs):
                ...
                for _ncol, _nname, _nstore in shell_name_pos_by_line.get(abs_line, []):
                    ...
                    if _nkind is None:
                        active, prior_kinds = _shell_active_for(
                    ...
                    if (
                        _ntag == "hpv"
                        and (abs_line, _ncol) in shell_instance_name_positions
                    ):
                        _ntag = "hpi"
                ...
                for _pcol, _pname, _pkind in shell_param_default_by_line.get(
                    abs_line, []
                ):
                    if (
                        _pkind == "var"
                        and (abs_line, _pcol) in shell_instance_name_positions
                    ):
                        _pkind = "instance"
                ...
                for _mcol, name in shell_module_literal_lines.get(abs_line, []):
                    ...
                    add_idx(
                        "hpm",
                        widx(abs_line, _mccol),
                        widx(abs_line, _mccol + len(name)),
                    )
                for dcol, dotted in shell_import_dotted_by_line.get(abs_line, []):
                    ...
                    for part in dotted.split("."):
                        add_idx(
                            "hpm", widx(abs_line, pos), widx(abs_line, pos + len(part))
                        )
                        pos += len(part) + 1
                for _ocol, _oname, _otag in shell_import_orig_by_line.get(abs_line, []):
                    ...
                    add_idx(
                        _otag,
                        widx(abs_line, _ocol),
                        widx(abs_line, _ocol + len(_oname)),
                    )
                ...
                for _tcol, _tattr, _tkind in shell_typed_attr_by_line.get(abs_line, []):
                    ...
                    add_idx(
                        _ttag,
                        widx(abs_line, _tcol),
                        widx(abs_line, _tcol + len(_tattr)),
                    )
            ...
            while pre_i < pre_n:
                ...
                if (
                    pch in ('"', "'")
                    and pre_i + 2 < pre_n
                    and shell_pre_text[pre_i + 1] == pch
                    and shell_pre_text[pre_i + 2] == pch
                ):
            ...
            while i < n:
                ...
                if (
                    ch in ('"', "'")
                    and i + 2 < n
                    and visible_code[i + 1] == ch
                    and visible_code[i + 2] == ch
                ):
    ...
    @property
    def running(self):
        """Whether the underlying Python process is still running."""
        return self.shellcmd.running

    def close(self):
        """Confirm before closing while the Python process is still
    ...
    def _cancel_all_after_ids(self):
        """Cancel the pending shell_setview() re-highlight callback, if any."""
...
def _init_pythonshell_hl_tags():
    """Reconfigure every open Python Shell buffer's highlight tags
