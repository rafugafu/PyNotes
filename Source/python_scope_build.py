"""The Python semantic-analysis engine behind editor.py's and
pythonshell.py's Python-HMode syntax highlighting, autocomplete, and
go-to-definition: static (AST-based, not executed) scope/type analysis
of a buffer's Python source, resolving what every name refers to
(local variable, parameter, function, class, imported module/name,
builtin, ...) well enough to highlight and autocomplete it correctly.
Also does limited resolution of imported modules' own members by
locating and statically inspecting their source files (or, for
extension/builtin modules with no source, introspecting the live
imported module), and best-effort static type inference (e.g. knowing
`x = "a"` makes x a str, so `x.` can autocomplete str's methods) via
_PYTHON_BUILTIN_METHOD_RETURNS below.

_python_build_scopes() is the main entry point, called by both
editor.py's Editor.ha()/python_trigger_name_scan() and
pythonshell.py's PythonShellBuffer._hapyshell_body()."""
...
# Reflection data about Python's own builtins, computed once at import
# time, used as a fallback/baseline wherever an imported module or
# instance's real members can't be determined more precisely: for each
# builtin type, every public member name and whether it's a method
# ("func") or attribute ("var").
...
def _python_build_builtin_callable_params():
    """Every builtin callable's keyword-capable parameter names (used
    to recognize/highlight `name=value` call arguments as keyword
    arguments for builtins, the same way it's done for user-defined
    ...
    for name in dir(builtins):
        ...
        kw = {
            ...
            if pp.kind in (pp.POSITIONAL_OR_KEYWORD, pp.KEYWORD_ONLY)
        }
        if kw:
            out[name] = kw
    return out
...
# A few builtins inspect.signature() can't introspect (implemented in
# C with no signature metadata), given their keyword parameters by hand.
for _bn, _bkw in {
    "min": {"key", "default"},
    "max": {"key", "default"},
    "int": {"base"},
...
_PYTHON_BUILTIN_CALLABLE_NAMES = {
    n for n in dir(_bmod) if callable(getattr(_bmod, n, None))
}
_PYTHON_BUILTIN_NAMES = set(dir(_bmod))
del _bmod
# For static type inference: what builtin type each of a builtin
# type's own methods returns (e.g. str.split() -> list), so a chained
# call/attribute access on the result can still be resolved to a type.
_PYTHON_BUILTIN_METHOD_RETURNS = {
    ...
    "bytearray": {
        ...
        "center": "bytearray",
        "expandtabs": "bytearray",
        "hex": "str",
        "join": "bytearray",
        "ljust": "bytearray",
        "lower": "bytearray",
        "lstrip": "bytearray",
        "removeprefix": "bytearray",
        "removesuffix": "bytearray",
        "replace": "bytearray",
        "rjust": "bytearray",
        "rstrip": "bytearray",
        "strip": "bytearray",
    ...
    "list": {"copy": "list", "count": "int", "index": "int"},
    "dict": {
        "copy": "dict",
        "keys": "list",
        "values": "list",
...
class _PythonScanCancelled(BaseException):
    """Raised to abort an in-progress scan (see _python_build_scopes())
    when the buffer has changed since it started, so a stale result
    isn't produced."""
...
class _PythonScopeBuilder(ast.NodeVisitor):
    """Walks a parsed buffer's AST once, collecting everything
    _python_build_scopes() needs into plain lists/dicts on self: every
    scope's line range and name bindings (self.scopes, built via
    push_scope()/add_name() as functions/classes/comprehensions are
    entered), every function/method's parameters (func_params,
    class-aware via add_args()' is_method), every import and aliasing
    of a name to a module/attribute, every call site's keyword
    arguments, and more (each accumulator named for what it holds).
    This is pure bookkeeping - it makes no attempt to resolve a name to
    what it actually refers to; that resolution happens afterward in
    _python_build_scopes(), which is what everything gathered here
    feeds into."""
    ...
    def add_name(self, name, lineno, kind):
        """Record a binding of name (kind: "var"/"func"/"class"/
        "func_arg"/"first_param"/"module"/...) at lineno in the current
        scope - or, if name was declared global/nonlocal in this scope,
        in the scope that declaration actually targets."""
        ...
        if name in sc.get("globals", {}):
            ...
            while _p is not None:
                _ps = self.scopes[_p]
                if (
                    _ps.get("kind") == "function"
                    and name not in _ps.get("globals", {})
                    and name not in _ps.get("nonlocals", {})
                ):
                    sc_idx = _p
                    break
                _p = _ps["parent"]
    ...
    def targets(self, node):
        """Flatten an assignment target (a Name, Attribute, or a
        Tuple/List/Starred of them, e.g. from `a, (b, *c) = ...`) into
        the individual Name/Attribute nodes actually being assigned."""
        out = []
    ...
    def push_scope(self, start, end, kind="function", start_col=0, end_col=None):
        """Open a new nested scope (function/class/lambda/comprehension)
        spanning lines start..end, and make it current; the caller is
        responsible for popping self.scope_stack when done visiting it."""
        idx = len(self.scopes)
        self.scopes.append(
            {
                "start": start,
    ...
    def add_args(self, args, lineno, is_method=False):
        """Bind every parameter in args (an ast.arguments) as a name in
        the current (just-pushed) function scope; is_method marks the
        first positional parameter as "first_param" (self/cls) rather
        than a regular "func_arg", so it can be highlighted/treated
        differently."""
        all_args = list(args.posonlyargs) + list(args.args) + list(args.kwonlyargs)
        for i, a in enumerate(all_args):
            self.add_name(
                a.arg, lineno, "first_param" if (is_method and i == 0) else "func_arg"
            )
        if args.vararg:
    ...
    def visit_function(self, node):
        """Shared visit_FunctionDef/visit_AsyncFunctionDef body: bind
        the function's name in the enclosing scope, record its
        parameters (func_params) and whether it accepts arbitrary
        keyword arguments (func_accepts_any, i.e. has **kwargs), visit
        its decorators/default values in the enclosing scope (where
        they're actually evaluated), then push a new scope for its
        parameters/body."""
        ...
        for a in list(node.args.args) + list(node.args.kwonlyargs):
            params.add(a.arg)
        self.func_params.setdefault(node.name, []).append(
            (node.lineno, params, self.scope_stack[-1])
        )
        self.func_accepts_any.setdefault(node.name, []).append(
            (node.lineno, bool(node.args.kwarg), self.scope_stack[-1])
        )
        ...
        is_method = self._in_class_body and _python_method_has_implicit_first_param(
            node
        )
    ...
    def _dotted_path(self, node):
        """The full dotted name of a chain of attribute accesses on a
        plain Name (e.g. Attribute(Attribute(Name('a'),'b'),'c') ->
        ['a','b','c']), or None if it isn't one (e.g. it's a call's
        result being accessed, not a plain name)."""
        parts = []
        cur = node
    ...
    def visit_Call(self, node):
        """Record every keyword argument (name=value) this call site
        passes, both for later validating them against the callee's
        actual parameters (pending_calls, resolved in
        _python_build_scopes()) and for highlighting the `name=`
        part (kwarg_positions)."""
        func_name = None
    ...
    def visit_ClassDef(self, node):
        """Bind the class's name, record any keyword arguments in its
        base-class list (e.g. `class C(Base, metaclass=M)`), visit
        decorators, then push a class scope for its body (tracking
        self._in_class_body so visit_function() above knows to treat
        the body's first def's first parameter as self/cls). Finally,
        if the class defines __init__, treat its parameters as the
        class's own "callable parameters" too, for validating/
        highlighting `ClassName(kwarg=...)` construction calls the same
        way a plain function call's keyword arguments are."""
        ...
        for stmt in node.body:
            if isinstance(stmt, ast.FunctionDef) and stmt.name == "__init__":
                ...
                params = set(a.arg for a in _iargs[1:]) - set(
                ...
                self.func_accepts_any.setdefault(node.name, []).append(
                    (node.lineno, bool(stmt.args.kwarg), self.scope_stack[-1])
                )
                self.func_params.setdefault(node.name, []).append(
                    (node.lineno, params, self.scope_stack[-1])
                )
    ...
    def visit_comp(self, node):
        """Shared body for list/set/dict comprehensions and generator
        expressions: they get their own scope (Python 3 semantics) for
    ...
    def _register_lambda(self, name, lineno, lam):
        """Record a lambda assigned directly to a name (`name = lambda
        ...: ...`) as if it were a def, so calls to name can have their
        keyword arguments validated/highlighted the same way."""
        _largs = lam.args
        _lparams = set(a.arg for a in list(_largs.args) + list(_largs.kwonlyargs))
        self.func_params.setdefault(name, []).append(
            (lineno, _lparams, self.scope_stack[-1])
        )
        self.func_accepts_any.setdefault(name, []).append(
            (lineno, bool(_largs.kwarg), self.scope_stack[-1])
        )

    def visit_Assign(self, node):
        """Bind every assignment target as a name, and, where the
        right-hand side is itself a simple Name/attribute/call worth
        tracking, record what the target is an alias of - a plain name
        (alias_assigns), a dotted attribute access
        (dotted_alias_assigns), a __import__()/importlib.import_module()
        call (dynamic_imports), or a functools.partial() of another
        callable (also alias_assigns, via _python_partial_target()) -
        so later attribute/call access on the target can be resolved
        through to whatever it's really an alias of. Also handles
        tuple/list unpacking assignment (`a, b = x, y`) element-wise."""
        ...
        for _ntgt in node.targets:
            ...
            if isinstance(val, ast.Name):
                self.alias_assigns.append(
                    (self.scope_stack[-1], node.lineno, tgt_name, val.id)
                )
            elif isinstance(val, ast.Attribute):
                self.dotted_alias_assigns.append(
                    (self.scope_stack[-1], node.lineno, tgt_name, val)
                )
            elif isinstance(val, ast.Call):
                _dr = self._dynamic_import_module(val)
                if _dr is not None:
                    self.dynamic_imports.append(
                        (self.scope_stack[-1], node.lineno, tgt_name, _dr[0], _dr[1])
                    )
                _pt = _python_partial_target(val)
                if _pt is not None:
                    self.alias_assigns.append(
                        (self.scope_stack[-1], node.lineno, tgt_name, _pt)
                    )
        for _ttgt in node.targets:
            if not (
                isinstance(_ttgt, (ast.Tuple, ast.List))
                and isinstance(node.value, (ast.Tuple, ast.List))
                and len(_ttgt.elts) == len(node.value.elts)
            ):
                continue
            for _tgt_el, _val_el in zip(_ttgt.elts, node.value.elts):
                ...
                if isinstance(_val_el, ast.Name):
                    self.alias_assigns.append(
                        (self.scope_stack[-1], node.lineno, _tgt_el.id, _val_el.id)
                    )
                elif isinstance(_val_el, ast.Attribute):
                    self.dotted_alias_assigns.append(
                        (self.scope_stack[-1], node.lineno, _tgt_el.id, _val_el)
                    )
    ...
    def _dynamic_import_module(self, call):
        """If call is a __import__(...) or importlib.import_module(...)
        call with a literal string module name, return (module_name,
        is_builtin_import) so visit_Assign() can track the target as an
        alias of that module even though it wasn't a plain `import`
        statement; None otherwise. (is_builtin_import is True for
        __import__(), which - unless a nonempty fromlist is given -
        actually binds the top-level package, not the full dotted
        submodule, unlike import_module().)"""
        fn = call.func
    ...
    def visit_for(self, node):
        """Shared visit_For/visit_AsyncFor body: bind the loop
        variable(s) (note: no new scope - a for loop's variables leak
        into the enclosing scope, unlike a comprehension's)."""
        for nm in self.targets(node.target):
            if not isinstance(nm, ast.Attribute):
                self.add_name(nm.id, node.lineno, "var")
    ...
    def visit_with(self, node):
        """Shared visit_With/visit_AsyncWith body: bind each `as`
        target."""
    ...
    def visit_ExceptHandler(self, node):
        """Bind `except ... as name`'s name, if given."""
        if node.name:
    ...
    def visit_Import(self, node):
        """Record each `import x[.y.z][ as name]`: the bound name
        (import_names, module_literals for highlighting), any aliasing
        (module_aliases and friends, for resolving later attribute
        access through the alias), and the dotted path's own positions
        (import_dotted_lines, so each dotted component can be
        highlighted as a module name)."""
        for alias in node.names:
            imported_name = alias.name
            top_name = imported_name.split(".")[0]
            used_name = alias.asname if alias.asname else top_name
            self.import_names.append(
                (self.scope_stack[-1], imported_name, top_name, used_name, node.lineno)
            )
            ...
            if alias.asname:
                ...
                self.module_alias_defs.setdefault(alias.asname, []).append(
                    (node.lineno, imported_name)
                )

    def visit_ImportFrom(self, node):
        """Record each `from module import name[ as alias]`
        (from_imports, resolved against module in
        _python_build_scopes()), plus the module path's own position
        for highlighting (import_dotted_lines) and, for a renamed
        import, the original name's position (import_orig_names, so it
        can still be tagged even though the bound name is different)."""
        if node.module:
            self.module_literals.append((node.lineno, node.module.split(".")[0]))
            if "." in node.module:
                _fcol = getattr(node, "col_offset", None)
                if _fcol is not None:
                    self.import_dotted_lines.append(
                        (node.lineno, _fcol + 5, node.module)
                    )
        for alias in node.names:
            name = alias.asname if alias.asname else alias.name
            self.from_imports.append(
                (self.scope_stack[-1], node.module, name, alias.name, node.lineno)
            )
            if alias.asname and alias.name != "*":
                _ocol = getattr(alias, "col_offset", None)
                if _ocol is not None:
                    self.import_orig_names.append(
                        (node.lineno, _ocol, alias.name, node.module)
                    )
...
def _python_method_has_implicit_first_param(node):
    """Whether a method def implicitly receives self/cls as its first
    parameter - true unless it's decorated @staticmethod."""
...
def _python_c3_linearize(cls, bases_map, seen, depth=0):
    """Compute cls's method resolution order via C3 linearization
    (the same algorithm Python itself uses for base-class order), given
    bases_map (each class name mapped to its direct base names), so
    attribute/method lookup on an instance can search base classes in
    the same order Python itself would. seen guards against a
    base-class cycle, depth against runaway recursion on malformed
    input."""
...
def _python_partial_target(call):
    """If call is a functools.partial(name, ...) call, return the
    wrapped callable's name; None otherwise. Lets a `p =
    partial(some_func, ...)` alias resolve through to some_func."""
    if not isinstance(call, ast.Call) or not call.args:
        return None
    _fn = call.func
    _isp = (isinstance(_fn, ast.Name) and _fn.id == "partial") or (
        isinstance(_fn, ast.Attribute) and _fn.attr == "partial"
    )
    if not _isp:
        return None
...
def _python_unwrap_descriptor(val):
    """Strip any staticmethod(...)/classmethod(...) wrapper call(s)
    around val, returning the underlying wrapped value/function."""
    while (
        isinstance(val, ast.Call)
        and isinstance(val.func, ast.Name)
        and val.func.id in _PYTHON_DESCRIPTOR_WRAPPERS
        and len(val.args) == 1
...
def _python_import_fromlist_is_nonempty(call):
    """Whether an __import__(...) call's fromlist argument (positional
    or keyword) is a non-empty list/tuple/set - if so, __import__
    returns the leaf submodule rather than the top-level package (see
    _PythonScopeBuilder._dynamic_import_module())."""
    _fl = None
    if len(call.args) >= 4:
        _fl = call.args[3]
    else:
...
def _python_static_value_kind(val, members, prefix):
    """Best-effort "func"/"class"/None classification of an assigned
    value AST node val (e.g. for `x = SomeClass` or `x = some_func`,
    used when statically inspecting a module's top-level assignments -
    see _python_inspect_ast_members()); members is the {name: kind}
    map already known for the containing scope/class (prefix), checked
    first before falling back to whether val's name is a known
    builtin."""
...
def _python_inspect_ast_members(node_list, prefix=""):
    """Statically build a {dotted_name: kind} map ("func"/"class"/
    "var") of every member defined at the top level of node_list (a
    module's or class's body), recursing into nested classes (each
    keyed under "{prefix}.{name}", so nested/class members get fully
    dotted keys). For a class, also scans every method body for
    `self.attr = value` assignments to discover instance attributes
    that aren't directly in the class body. A class decorated with
    enum's @global_enum also gets its own members re-exposed at the
    containing scope, matching global_enum's real runtime effect of
    injecting them into the module namespace."""
    members = {}
    for node in node_list:
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            for _sub in node.body:
                if (
                    isinstance(_sub, (ast.FunctionDef, ast.AsyncFunctionDef))
                    and _sub.args.args
                    and _python_method_has_implicit_first_param(_sub)
                ):
                    _sfp = _sub.args.args[0].arg
                    for _sst in ast.walk(_sub):
                        _stgts = (
                            _sst.targets
                            if isinstance(_sst, ast.Assign)
                            else (
                                [_sst.target]
                                if isinstance(_sst, (ast.AnnAssign, ast.AugAssign))
                                else []
                            )
                        )
                        _sval = (
                            _sst.value
                            if isinstance(_sst, (ast.Assign, ast.AnnAssign))
                            else None
                        )
                        for _st in _stgts:
                            ...
                            for _ste in (
                                _st.elts
                                if isinstance(_st, (ast.Tuple, ast.List))
                                else [_st]
                            ):
                                if isinstance(_ste, ast.Starred):
                                    _ste = _ste.value
                                if (
                                    isinstance(_ste, ast.Attribute)
                                    and isinstance(_ste.value, ast.Name)
                                    and _ste.value.id == _sfp
                                ):
                                    _skind = (
                                        _python_static_value_kind(
                                            _sval, members, prefix
                                        )
                                        if _sdirect and _sval is not None
                                        else None
                                    ) or "var"
                                    members.setdefault(f"{key}.{_ste.attr}", _skind)
            if not prefix:
                is_global_enum = any(
                    (isinstance(d, ast.Name) and d.id == "global_enum")
                    or (isinstance(d, ast.Attribute) and d.attr == "global_enum")
            ...
            for target in node.targets:
                for _tel in (
                    ...
                    if isinstance(_tel, ast.Name):
                        ...
                        members[key] = (
                            _python_static_value_kind(node.value, members, prefix)
                            if _tel is target
                            else None
                        ) or "var"
            ...
            if isinstance(node.target, ast.Name):
                ...
                members[key] = (
                    _python_static_value_kind(node.value, members, prefix) or "var"
                )
...
class _PythonModuleSpec:
    """A minimal stand-in for importlib.machinery.ModuleSpec, as
    returned by _python_resolve_toplevel_fs()/_python_resolve_spec_fs():
    just enough (origin path, package search locations) for this module
    to locate and statically inspect the module's source, without
    actually using importlib's own (heavier, import-triggering) spec
    machinery."""
...
def _python_resolve_toplevel_fs(name):
    """Locate a top-level module/package named name by searching
    sys.path directly on the filesystem (a package's __init__.py, a
    namespace-package directory, a single .py file, or a compiled
    extension module), falling back to marking it as a built-in if
    it's in sys.builtin_module_names. Returns a _PythonModuleSpec, or
    None if it can't be found at all."""
...
def _python_module_src_path(spec, name):
    """The .py source file path for spec, if it has one directly
    (spec.origin already is one) or can be found on sys.path by name -
    None for a built-in/extension module with no Python source to
    statically inspect."""
...
def _python_relative_import_target(name, level, module, is_package):
    """Resolve a relative `from .[.[.]]module import ...` (level = the
    number of leading dots, module = the name after them, possibly
    empty) against the importing module's own dotted name, the same
    way Python's import system does."""
    if level == 0:
        return module
    base = name.split(".")
...
def _python_reset_scan_state(buf):
    """Wipe buf's Python-scan results back to empty (e.g. when its
    content is replaced wholesale - loading a new file, disconnecting
    from a peer view) and, if it's in Python HMode, kick off a fresh
    scan; bumping the edit generation cancels any scan already in
    flight for the old content (see _PythonScanCancelled)."""
...
def _python_find_spec_cached(buf, name):
    """_python_resolve_spec_fs(buf, name), cached per-buffer
    (buf._python_module_spec_cache) since resolving the same import
    repeatedly on every scan would be wasteful."""
...
def _python_resolve_spec_fs(buf, name):
    """Locate a (possibly dotted, e.g. "a.b.c") module by resolving
    each dotted component's parent package (recursively, via
    _python_find_spec_cached()) and then searching its
    submodule_search_locations for the next one. Returns a
    _PythonModuleSpec, or None if any component can't be found."""
    parts = name.split(".")
...
def _python_resolve_module_members(buf, name, visited=None):
    """Statically resolve module name's top-level members - {member:
    kind} ("func"/"class"/"var"/"module", plus a paired
    "@modtarget:member" entry recording which real module a re-exported
    "module" member actually points to - by parsing its source (no
    execution) via _python_inspect_ast_members(), then folding in
    whatever it imports and re-exports at module scope: `import x`,
    `from x import y[ as z]`, and `from x import *` (each resolved
    recursively). "Module scope" here also covers imports made inside
    an if/try/with/for block at the top level, or inside a function
    that then `global`-declares the imported name(s), matching what
    actually ends up bound in the module's namespace; imports inside a
    class body or an ordinary (non-global) function don't count.
    Cached per-buffer (buf._python_module_members_cache); visited
    guards against an import cycle between modules."""
    ...
    def _collect_scope_imports(_stmts, _globals, _depth=0):
        """Recurse into _stmts (module-scope-reachable code:
        if/try/with/for bodies, or a function body once _globals is
        known), collecting into _import_nodes every import that ends
        up bound at module scope - either directly (_globals is None,
        meaning _stmts is already module-level) or via a `global`
        declaration inside a function (_globals is the set of names
        that function declared global, tracked as it's entered).
        Imports inside a class body are excluded outright (a fresh
        empty _globals is passed for those, which no import can match)."""
        if _depth > 60:
            return
        for _st in _stmts:
        ...
        if isinstance(node, ast.ImportFrom):
            sub_name = (
                _python_relative_import_target(
                    ...
                    node.level,
                    node.module,
                    bool(getattr(spec, "submodule_search_locations", None)),
                )
            ...
            for alias in node.names:
                if alias.name == "*":
                    if sub_members is None:
                        sub_members = _python_resolve_module_members(
                            buf, sub_name, visited
                        )
                    ...
                    if exported not in members:
                        ...
                        if alias.name in sub_members:
                            ...
                            _python_find_spec_cached(buf, f"{sub_name}.{alias.name}")
                            is not None
                        ):
                            members[exported] = "module"
                            members["@modtarget:" + exported] = (
                                f"{sub_name}.{alias.name}"
                            )
...
def _python_resolve_module_member_kind(buf, mod, class_name, member, seen=None):
    """Resolve the kind ("func"/"class"/"var"/None) of
    mod.class_name.member, searching class_name's own members first,
    then (if class_name is itself an alias imported from elsewhere)
    through to the real class, then up through class_name's own base
    classes (however they're spelled - a plain name, a dotted path
    through an imported module, ...) the same way Python's own
    attribute lookup would. seen prevents infinite recursion through a
    base-class/import cycle."""
    if seen is None:
        seen = set()
    key = (mod, class_name)
    if key in seen:
    ...
    if (
        "@bases:" + class_name not in fp
        and class_name in imports
        and imports[class_name][1] is not None
    ):
        return _python_resolve_module_member_kind(
            buf, imports[class_name][0], imports[class_name][1], member, seen
        )
    for base in fp.get("@bases:" + class_name, []):
        ...
        if len(bparts) == 1:
            if "@bases:" + base in fp:
                ...
                _r = _python_resolve_module_member_kind(
                    buf, imports[base][0], imports[base][1], member, seen
                )
                if _r is not None:
                    return _r
        else:
            ...
            if broot in imports:
                ...
                _r = _python_resolve_module_member_kind(
                    buf, full_mod, bparts[-1], member, seen
                )
                if _r is not None:
                    return _r
    return None
...
def _python_resolve_module_class_members(buf, mod, class_name, seen=None):
    """Like _python_resolve_module_member_kind() but resolves every
    member of mod.class_name at once (its own members plus, through
    the same alias/base-class chasing, everything inherited), returning
    a {member: kind} map. Cached per (mod, class_name) at the top-level
    call (buf._python_module_class_members_cache); seen (only set on a
    recursive call) guards against a cycle."""
    ...
    if (
        ...
        for k, v in _python_resolve_module_class_members(
            buf, imports[class_name][0], imports[class_name][1], seen
        ).items():
            out.setdefault(k, v)
    for base in fp.get("@bases:" + class_name, []):
        bparts = base.split(".")
        if len(bparts) == 1:
            if "@bases:" + base in fp:
                ...
                for k, v in _python_resolve_module_class_members(
                    buf, imports[base][0], imports[base][1], seen
                ).items():
                    out.setdefault(k, v)
        else:
            broot = bparts[0]
...
def _python_build_scopes(
    ...
    seed_aliases=None,
    seed_origins=None,
    seed_method_params=None,
    seed_accepts_any=None,
    seed_module_bases=None,
    seed_func_origins=None,
    seed_attr_types=None,
    seed_class_attr_types=None,
    seed_func_params=None,
    seed_func_accepts_any=None,
    seed_class_bases=None,
    seed_inherited=None,
    seed_instance_only=None,
):
    """The whole module's main entry point, and its single largest
    piece of code (see the module docstring at the top of this file
    for the overall picture). At a high level:

    1. Parse text into an AST, tolerating syntax errors: since text is
       a live editor buffer, it's very often syntactically incomplete
       mid-edit (e.g. `if x:` with nothing after it yet), so a
       SyntaxError doesn't abort - the offending line(s) are patched
       (blanked/stubbed) and parsing is retried, up to once per line,
       so the rest of the buffer still gets analyzed.
    2. Walk the resulting tree once with _PythonScopeBuilder to collect
       raw facts (scopes, name bindings, imports, aliases, call sites'
       keyword arguments, ...).
    3. Resolve those raw facts into what's actually needed for
       highlighting/autocomplete: what kind each name binding is
       (var/func/class/module/instance/...), what a name aliased to an
       import or another name really refers to (chasing through
       _python_resolve_module_members()/_python_resolve_module_member_
       kind() for real imported modules), best-effort static type
       inference for variables (so `x = "s"` then `x.` can autocomplete
       str's members, per _PYTHON_BUILTIN_METHOD_RETURNS), and which
       call sites' keyword arguments are valid for the function/class/
       method being called.
    4. Every seed_* parameter carries forward already-known facts from
       a previous call (an interactive Python Shell session
       remembering names bound by earlier statements - see
       pythonshell.PythonShellBuffer._hapyshell_body(), the other
       caller of this function besides editor.py); a plain editor
       buffer, which has no such prior session, leaves them None.

    gen (the buffer's current _python_edit_generation) and
    buf._python_edit_generation together let a long-running scan notice
    mid-way (via _ck() below) that the buffer has since changed and
    abort (raising _PythonScanCancelled) rather than produce a stale
    result - important since this can run on a background thread while
    the user keeps typing.

    The return value is a large fixed-order tuple of every resolved
    fact (scopes, call_kwargs, name/type positions, import info, class
    hierarchies, etc.); see the `return` statement at the bottom of
    this function for the exact order, and editor.py's Editor.ha()/
    pythonshell.py's _hapyshell_body() for how each piece is actually
    used to highlight/autocomplete.
    """
    ...
    for _ in range(len(lines) + 2):
        ...
        try:
            ...
            if "unexpected indent" in msg and ln > 1:
                ...
                earlier_same = any(
                    lines[i].strip()
                    and (len(lines[i]) - len(lines[i].lstrip())) == indent_n
                ...
                if earlier_same:
                    for i in range(ln - 2, -1, -1):
                        if lines[i].strip():
                            ...
                            ) < indent_n and not lines[i].rstrip().endswith(":"):
                                target = i
                            break
    ...
    if tree is None:
        return None
    _ck()
    # Phase 2: collect raw facts from the (possibly error-recovered)
    # tree, then seed the module scope with whatever a prior session
    # (see the seed_* docs above) already established, as if each
    # seeded name/alias had been bound at (fake) line 0.
        ...
        while sidx is not None:
            ...
            if name in sc["names"]:
                ...
                if (
                    ...
                    and name in _PYTHON_BUILTIN_NAMES
                ):
                    return None
                if (
                    best is None
                    and latest is not None
                    and _same_block(latest[0], lineno)
                ):
                    best = latest
    ...
    local_classes = {}
    class_module_origin = (
        {_so: [(0, _sv)] for _so, _sv in seed_origins.items()} if seed_origins else {}
    )
    ...
    for _tn in ast.walk(tree):
        ...
        if isinstance(_tn, ast.Name):
            name_positions.append(
                (_tn.lineno, _tn.col_offset, _tn.id, isinstance(_tn.ctx, ast.Store))
            )
            ...
            isinstance(_tn, ast.AnnAssign)
            and _tn.value is not None
            and isinstance(_tn.target, ast.Name)
        ):
            _tn.targets = [_tn.target]
            tree_assigns.append(_tn)
            ...
            if _tn.asname:
                name_positions.append(
                    (
                        _tn.end_lineno,
                        _tn.end_col_offset - len(_tn.asname.encode("utf-8")),
                        _tn.asname,
                        True,
                    )
                )
            ...
            if (
                _tn.name
                and _tn.type is not None
                and 0 < _tn.type.end_lineno <= len(lines)
            ):
            ...
            if 0 < _tn.lineno <= len(lines):
                ...
                for _gname in _tn.names:
                    ...
                    global_stmt_positions.append(
                        (_tn.lineno, _i, _gname, _scope_for_line(_tn.lineno))
                    )
                    _i += len(_nb)
    def_name_positions = []
    for _tn in tree_class_defs + tree_func_defs:
        ...
        _nb = _tn.name.encode("utf-8")
        if _enc[_i : _i + len(_nb)] == _nb:
            def_name_positions.append(
                (
                    _tn.lineno,
                    _i,
                    _tn.name,
                    "class" if isinstance(_tn, ast.ClassDef) else "func",
                )
            )
    _class_def_by_name = {}
    ...
    def _compute_class_members(node):
        ...
        for _bstmt in _flatten_class_body(node.body):
            _btgts = (
                ...
                else ([_bstmt.target] if isinstance(_bstmt, ast.AnnAssign) else [])
            )
            _bval = (
                _bstmt.value
                if isinstance(_bstmt, (ast.Assign, ast.AnnAssign))
                else None
            )
            ...
            if not isinstance(_bval, ast.Name):
                continue
            for _btgt in _btgts:
                if isinstance(_btgt, ast.Name) and members.get(_btgt.id) in (
                    "var",
                    None,
                ):
        ...
        for meth in node.body:
            if isinstance(meth, (ast.FunctionDef, ast.AsyncFunctionDef)):
                _fp = (
                    meth.args.args[0].arg
                    if (
                        meth.args.args and _python_method_has_implicit_first_param(meth)
                    )
                    else None
                )
                for stmt in ast.walk(meth):
                    ...
                    for tgt, _tval in _assign_pairs(
                        tgts,
                        (
                            stmt.value
                        ...
                        if (
                            ...
                            and tgt.value.id == _fp
                        ):
                            _mvk = "var"
                            if isinstance(_tval, ast.Lambda):
                                _mvk = "func"
                            elif (
                                isinstance(_tval, ast.Attribute)
                                and isinstance(_tval.value, ast.Name)
                                and _tval.value.id == _fp
                            ):
                                ...
                                if _nvk in ("func", "class", "module"):
                                    _mvk = _nvk
                            if _mvk == "var" and members.get(tgt.attr) in (
                                "func",
                                "class",
                                "module",
                            ):
    ...
    for _tn in tree_class_defs:
        ...
        class_def_scopes[(_tn.name, _tn.lineno)] = _binding_scope_for(
            _tn.name, _binding_scope_of_def(_tn.lineno)
        )
    ...
    def _class_def_line_at(name, lineno):
        ...
        for _sidx in _def_scope_chain(lineno):
            _cands = [
                _cl for _cl in _lns if class_def_scopes.get((name, _cl), 0) == _sidx
            ]
            if not _cands:
                continue
            _best = None
    ...
    for scope in builder.scopes:
        for name, defs in scope["names"].items():
            for _, kind in defs:
                if (
                    kind == "class"
                    and name in _class_def_by_name
                    and name not in local_classes
                ):
                    try:
    ...
    def _node_class_key(node):
        if node.name in local_classes and node.lineno != _class_first_line.get(
            node.name
        ):
    ...
    for node in tree_class_defs:
        if node.name in local_classes:
            ...
            _class_body_members[_node_class_key(node)] = set(
                _bm for _bm in _python_inspect_ast_members(node.body) if "." not in _bm
            )
        ...
        if node.name in local_classes:
            ...
            for sub in node.body:
                if isinstance(sub, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    ...
                    _margs = (
                        list(sub.args.posonlyargs)
                        + list(sub.args.args)
                        + list(sub.args.kwonlyargs)
                    )
                    _mp = (
                        set(a.arg for a in (_margs[1:] if _mimp else _margs))
                        - set(a.arg for a in sub.args.posonlyargs)
                    ) | set(a.arg for a in sub.args.kwonlyargs)
                    if sub.args.kwarg:
                    ...
                    if _sfp:
                        for _astmt in ast.walk(sub):
                            ...
                            for _atg in _astmt.targets:
                                if (
                                    isinstance(_atg, ast.Attribute)
                                    and isinstance(_atg.value, ast.Name)
                                    and _atg.value.id == _sfp
                                ):
                                    _av = _astmt.value
                                    if isinstance(_av, ast.Lambda):
                                        _lp = set(
                                            a.arg
                                            for a in list(_av.args.args)
                                            + list(_av.args.kwonlyargs)
                                        )
                                        local_class_method_params[
                                            _nk + "." + _atg.attr
                                        ] = _lp
                                        if _av.args.kwarg:
                                            local_class_accepts_any.add(
                                                _nk + "." + _atg.attr
                                            )
                                    elif (
                                        isinstance(_av, ast.Attribute)
                                        and isinstance(_av.value, ast.Name)
                                        and _av.value.id == _sfp
                                    ):
                                        _class_attr_lambda_params.setdefault(
                                            _nk, []
                                        ).append((_atg.attr, _av.attr))
                for _clstmt in _flatten_class_body(node.body):
                    _clval = (
                        _clstmt.value
                        if isinstance(_clstmt, (ast.Assign, ast.AnnAssign))
                        else None
                    )
                    ...
                    _cltgts = (
                        _clstmt.targets
                        if isinstance(_clstmt, ast.Assign)
                        else [_clstmt.target]
                    )
                    _clargs = (
                        list(_clval.args.posonlyargs)
                        + list(_clval.args.args)
                        + list(_clval.args.kwonlyargs)
                    )
                    _clp = (
                        set(a.arg for a in (_clargs[1:] if _clargs else _clargs))
                        - set(a.arg for a in _clval.args.posonlyargs)
                    ) | set(a.arg for a in _clval.args.kwonlyargs)
                ...
                for _cmstmt in _flatten_class_body(node.body):
                    _cmval = (
                        _cmstmt.value
                        if isinstance(_cmstmt, (ast.Assign, ast.AnnAssign))
                        else None
                    )
                    ...
                    _cmtgts = (
                        _cmstmt.targets
                        if isinstance(_cmstmt, ast.Assign)
                        else [_cmstmt.target]
                    )
                    for _cmtgt in _cmtgts:
                        if not isinstance(_cmtgt, ast.Name):
                            continue
                        local_class_method_params[_nk + "." + _cmtgt.id] = (
                            local_class_method_params[_cmsrc]
                        )
                ...
                for _cmstmt in _flatten_class_body(node.body):
                    ...
                    _cmval = _python_unwrap_descriptor(_cmval)
                    if not (
                        isinstance(_cmval, ast.Attribute)
                        and isinstance(_cmval.value, ast.Name)
                    ):
                        continue
                    _cmbase = _class_key_at(_cmval.value.id, _cmstmt.lineno)
                    _cmsrc = _cmbase + "." + _cmval.attr
                    if (
                        _cmbase not in local_classes
                        or _cmsrc not in local_class_method_params
                    ):
                        continue
                    _cmtgts = (
                        _cmstmt.targets
    ...
    for _ncnode in tree_class_defs:
        ...
        for _ncsub in _ncnode.body:
            if isinstance(_ncsub, ast.ClassDef) and _ncsub.name in local_classes:
                class_attr_types.setdefault(_nckey, {})[_ncsub.name] = (
                    "class",
                    _node_class_key(_ncsub),
                )
            ...
            for base in _python_c3_linearize(cls, class_bases, frozenset())[1:]:
                ...
                for k, v in local_classes[base].items():
                    if (base, k) in _inherited_members or k in _filled:
                        continue
                    if k not in local_classes[cls] or (
                        local_classes[cls][k] == "var" and v != "var"
                    ):
                ...
                for _ak, _av in class_attr_types.get(base, {}).items():
                    if (base, _ak) in _inherited_attr_types or _ak in _filled_attrs:
                        continue
                    if (
                        _ak in _self_assigned_attrs.get(cls, ())
                        or (cls, _ak) in _direct_attr_assigns
                    ):
                        continue
                ...
                for _mk in list(local_class_method_params):
                    if (
                        not _mk.startswith(base + ".")
                        or _mk in _inherited_method_params
                    ):
                        continue
    ...
    for _calp_nk, _calp_pairs in _class_attr_lambda_params.items():
        for _attr_name, _src_meth in _calp_pairs:
            ...
            if (
                _src_key in local_class_method_params
                and _calp_nk + "." + _attr_name not in local_class_method_params
            ):
                local_class_method_params[_calp_nk + "." + _attr_name] = (
                    local_class_method_params[_src_key]
                )
    ...
    for scope_idx, imported_name, top_name, used_name, lineno in builder.import_names:
        if used_name == top_name:
            if top_name in valid_modules:
                builder.scopes[scope_idx]["names"].setdefault(used_name, []).append(
                    (lineno, "module")
                )
    ...
    for _nlp, _nln in builder.nonlocal_seeds:
        ...
        while _nlcur is not None:
            if builder.scopes[_nlcur].get("kind") == "function" and builder.scopes[
                _nlcur
            ]["names"].get(_nln):
                _nlbound = True
                break
        ...
        if not _nlbound:
            builder.scopes[_nlp]["names"].setdefault(_nln, []).append(
                (builder.scopes[_nlp]["start"], "var")
            )
    for (
        scope_idx,
        module_name,
        ...
        if imported_name == "*":
            for _wname, _wkind in contents.items():
                if "." not in _wname:
                    builder.scopes[scope_idx]["names"].setdefault(_wname, []).append(
                        (lineno, _wkind)
                    )
        else:
            if _orig_name in contents:
                ...
                if kind == "module":
                    ...
                    if (
                        _fmtgt not in valid_modules
                        and _python_find_spec_cached(buf, _fmtgt) is not None
                    ):
                        valid_modules.add(_fmtgt)
                        _fmmems = _python_resolve_module_members(buf, _fmtgt)
                        if _fmmems:
                            module_contents[_fmtgt] = _fmmems
                    builder.module_alias_defs.setdefault(imported_name, []).append(
                        (lineno, _fmtgt)
                    )
                ...
                f"{module_name}.{_orig_name}" in valid_modules
                or _python_find_spec_cached(buf, f"{module_name}.{_orig_name}")
                is not None
            ):
                kind = "module"
                builder.module_alias_defs.setdefault(imported_name, []).append(
                    (lineno, f"{module_name}.{_orig_name}")
                )
                if lineno >= builder.module_alias_lines.get(imported_name, 0):
                    builder.module_aliases[imported_name] = (
                        f"{module_name}.{_orig_name}"
                    )
                    builder.module_alias_lines[imported_name] = lineno
            else:
                continue
            builder.scopes[scope_idx]["names"].setdefault(imported_name, []).append(
                (lineno, kind)
            )
            ...
            for _wname, _wkind in mc.items():
                if "." not in _wname and _wkind == "class":
                    if _wname not in local_classes:
                        _wmems = _python_resolve_module_class_members(
                            buf, module_name, _wname
                        )
                        if _wmems:
                            local_classes[_wname] = _wmems
                    if _wname in local_classes:
                        class_module_origin.setdefault(_wname, []).append(
                            (_fln0, (module_name, _wname))
                        )
        elif mc.get(_orig_name) == "class":
            if imported_name not in local_classes:
                _imp_mems = _python_resolve_module_class_members(
                    buf, module_name, _orig_name
                )
                if _imp_mems:
                    local_classes[imported_name] = _imp_mems
            if imported_name in local_classes:
                class_module_origin.setdefault(imported_name, []).append(
                    (_fln0, (module_name, _orig_name))
                )
    base_to_module = {}
    for _, imported_name, top_name, used_name, _iln in builder.import_names:
        if used_name == top_name:
            ...
            base_to_module.setdefault(used_name, []).append(
                (_iln, _real_module_name(imported_name))
            )
    ...
    for _alias, _adefs in builder.module_alias_defs.items():
        for _al, _afull in _adefs:
            if _afull in valid_modules or _afull in _dotted_module_targets:
                base_to_module.setdefault(_alias, []).append(
                    (_al, _real_module_name(_afull))
                )
    ...
    for _dsc, _dln, _dname, _dmod, _dbuiltin in builder.dynamic_imports:
        ...
        if _python_find_spec_cached(buf, _dmod) is not None:
            ...
            _dnames[:] = [
                (_l, _k) for _l, _k in _dnames if not (_l == _dln and _k == "var")
            ]
    ...
    for _asc, _aln, _aname, _asrc in builder.alias_assigns:
        ...
        if _srckind is None and _asrc in _PYTHON_BUILTIN_MEMBERS:
            ...
            if _asrc in _PYTHON_BUILTIN_CALLABLE_PARAMS:
                builder.func_params.setdefault(_aname, []).append(
                    (
                        _aln,
                        _PYTHON_BUILTIN_CALLABLE_PARAMS[_asrc],
                        _binding_scope_for(_aname, _asc),
                    )
                )
        if _srckind == "module":
            ...
            if _asrc_mod is not None:
                ...
                _anames[:] = [
                    (_l, _k) for _l, _k in _anames if not (_l == _aln and _k == "var")
                ]
                _anames.append((_aln, "module"))
                base_to_module.setdefault(_aname, []).append((_aln, _asrc_mod))
                builder.module_alias_defs.setdefault(_aname, []).append(
                    (_aln, _asrc_mod)
                )
        if _srckind in ("class", "func", "builtin"):
            ...
            if _srckind in ("func", "builtin"):
                ...
                if _src_params is not None:
                    builder.func_params.setdefault(_aname, []).append(
                        (_aln, _src_params, _absc)
                    )
                    builder.func_accepts_any.setdefault(_aname, []).append(
                        (_aln, _func_accepts_any_at(_asrc, _aln), _absc)
                    )
            if _srckind in ("class", "builtin"):
                ...
                if _asrc_key in local_classes:
                    ...
                    class_type_maps[_adst_key] = class_type_maps.setdefault(
                        _asrc_key, {}
                    )
                    _sorig = _class_origin_at(_asrc, _aln)
                    if _sorig is not None:
                        class_module_origin.setdefault(_aname, []).append(
                            (_aln, _sorig)
                        )
                    for _mk in list(local_class_method_params):
                        if _mk.startswith(_asrc_key + "."):
                            _ameth = _mk[len(_asrc_key) + 1 :]
                            local_class_method_params[_adst_key + "." + _ameth] = (
                                local_class_method_params[_mk]
                            )
                            if _mk in local_class_accepts_any:
    ...
    def _resolve_dotted_alias(base, attrs, lineno):
        ...
        for _i, _attr in enumerate(attrs):
            ...
            _kind = _mm.get(_attr)
            _sub = f"{mod}.{_attr}"
            if _kind is None and (
                _sub in valid_modules or _python_find_spec_cached(buf, _sub) is not None
            ):
                _kind = "module"
            if _kind == "module":
    ...
    for _cbnode in tree_class_defs:
        ...
        for _cbstmt in _flatten_class_body(_cbnode.body):
            _cbtgts = (
                _cbstmt.targets
                if isinstance(_cbstmt, ast.Assign)
                else ([_cbstmt.target] if isinstance(_cbstmt, ast.AnnAssign) else [])
            )
            _cbval = (
                _cbstmt.value
                if isinstance(_cbstmt, (ast.Assign, ast.AnnAssign))
                else None
            )
            ...
            for _cbtgt in _cbtgts:
                ...
                if _cbr[0] == "localclass":
                    ...
                    class_attr_types.setdefault(_cbkey, {})[_cbtgt.id] = (
                        "class",
                        _cbr[1],
                    )
                elif _cbr[0] == "class":
    ...
    for _daa_sc, _daa_ln, _daa_name, _daa_val in builder.dotted_alias_assigns:
        ...
        _anames[:] = [
            (_l, _k) for _l, _k in _anames if not (_l == _daa_ln and _k == "var")
        ]
        ...
        if _akind == "localclass":
            ...
            for _mk in list(local_class_method_params):
                if _mk.startswith(_asrckey + "."):
                    ...
                    if _akey + "." + _ameth not in local_class_method_params:
                        local_class_method_params[_akey + "." + _ameth] = (
                            local_class_method_params[_mk]
                        )
                        if _mk in local_class_accepts_any:
            ...
            name_module_class.setdefault(_daa_name, []).append(
                (_daa_ln, (_amod, _aattr))
            )
            class_def_lines.setdefault(_daa_name, [])
            if _daa_ln not in class_def_lines[_daa_name]:
                class_def_lines[_daa_name].append(_daa_ln)
            ...
            class_module_origin.setdefault(_daa_name, []).append(
                (_daa_ln, (_amod, _aattr))
            )
        elif _akind == "module":
            _asub = _dr[1]
    ...
    for _, module_name, imported_name, _orig_name, _fln in builder.from_imports:
        ...
        if module_name and imported_name != "*" and module_name in valid_modules:
            from_func_module.setdefault(imported_name, []).append(
                (_fln, (module_name, _orig_name))
            )
    for _afo_name, _afo_ln, _afo_mod, _afo_attr in _attr_alias_func_origins:
        from_func_module.setdefault(_afo_name, []).append(
            (_afo_ln, (_afo_mod, _afo_attr))
        )
    local_def_lines = {}
    ...
    def _lookup_module_callable(mod_name, key, seen=None):
        ...
        if (
            mod_name not in valid_modules
            and _python_find_spec_cached(buf, mod_name) is None
        ):
            return None, None
        ...
        if _head in imports and imports[_head][1] is not None:
            ...
            return _lookup_module_callable(
                _rmod, _rname if _tail is None else f"{_rname}.{_tail}", seen
            )
        return None, None
    ...
    def _resolve_through_module(mod_name, rest):
        ...
        while idx < len(rest):
            ...
            if (
                sub_full in valid_modules
                or _python_find_spec_cached(buf, sub_full) is not None
            ):
                cur_mod = sub_full
                idx += 1
                continue
    ...
    def _params_for_dotted(dotted, lineno):
        ...
        if len(dotted) == 1:
            ...
            for _fl, _fv in from_func_module.get(root, []):
                if _fl <= lineno and (_imp_line is None or _fl >= _imp_line):
                    _imp_line = _fl
                    _imp = _fv
            if _local_line is not None and (
                _imp_line is None or _local_line >= _imp_line
            ):
                if (
                    kind == "class"
                    and root in local_classes
                    and root in class_def_lines
                ):
                    ...
                    for _lcmo_mod, _lcmo_cls in local_class_module_origins.get(
                        _ckey, []
                    ):
                        found, mp = _python_resolve_module_method(
                            buf, _lcmo_mod, _lcmo_cls, "__init__"
                        )
                        if found:
                            return True, mp
                    ...
                    if _corig is not None:
                        found, mp = _python_resolve_module_method(
                            buf, _corig[0], _corig[1], "__init__"
                        )
                        if found:
                            return True, mp
                    return True, set()
        ...
        if len(rest) == 1 and _name_is_class_at(root, lineno):
            ...
            if local_classes.get(_ckey, {}).get(rest[0]) == "class":
                ...
                if _cat is not None and _cat[0] == "class" and _cat[1] in local_classes:
                    ...
                    if _torig is not None:
                        found, mp = _python_resolve_module_method(
                            buf, _torig[0], _torig[1], "__init__"
                        )
                        if found:
                            return True, mp
                    return True, set()
            ...
            for _lcmo_mod, _lcmo_cls in local_class_module_origins.get(_ckey, []):
                found, mp = _python_resolve_module_method(
                    buf, _lcmo_mod, _lcmo_cls, rest[0]
                )
                if found:
                    return True, mp
    ...
    for _ifn in tree_importfroms:
        if _ifn.module and "." not in _ifn.module and _ifn.module in valid_modules:
            module_literals.append(
                (_ifn.lineno, _ifn.col_offset + 5 + _ifn.level, _ifn.module)
            )
    ...
    import_dotted_lines = [
        (lineno, col, dotted)
        for lineno, col, dotted in builder.import_dotted_lines
        if dotted in valid_modules
        or dotted in _dotted_module_targets
        or _python_find_spec_cached(buf, dotted) is not None
    ]
    ...
    for _oln, _ocol, _oname, _omod in builder.import_orig_names:
        ...
        _okind = None
        if (
            f"{_omod}.{_oname}" in valid_modules
            or _python_find_spec_cached(buf, f"{_omod}.{_oname}") is not None
        ):
            _okind = "module"
        else:
            _ocontents = module_contents.get(_omod) or _python_resolve_module_members(
                buf, _omod
            )
    ...
    def _elem_type_name(v, lineno):
        ...
        if (
            ...
            if (
                _emod is not None
                and _python_resolve_module_members(buf, _emod).get(v.func.attr)
                == "class"
            ):
                class_module_origin.setdefault(v.func.attr, []).append(
                    (lineno, (_emod, v.func.attr))
                )
                return v.func.attr
        return None
        ...
        for tgt, val in _assign_pairs(node.targets, node.value):
            ...
            if isinstance(val, ast.Call):
                ...
                if isinstance(func, ast.Name):
                    type_name = func.id
                    mod_name = _line_def_at(
                        _from_import_map.get(type_name), node.lineno
                    )
                elif isinstance(func, ast.Attribute):
                    type_name = func.attr
                    mod_name = (
                        _base_module_at(func.value.id, node.lineno)
                        if isinstance(func.value, ast.Name)
                        else None
                    )
                if type_name and type_name not in local_classes and mod_name:
                    if (
                        _python_resolve_module_members(buf, mod_name).get(type_name)
                        == "class"
                    ):
                        class_module_origin.setdefault(type_name, []).append(
                            (node.lineno, (mod_name, type_name))
                        )
            ...
            if (
                ...
                and isinstance(val.func, ast.Name)
                and type_name
                and type_name in local_classes
                and not _name_is_class_at(type_name, node.lineno)
            ):
                type_name = None
            if (
                isinstance(val, ast.Call)
            ...
            if type_name and (
                type_name in local_classes
                or _class_origin_at(type_name, node.lineno) is not None
                or (
                    type_name in _PYTHON_BUILTIN_MEMBERS
                    and type_name not in local_classes
                )
            ):
                sc_idx = _binding_scope_for(tgt.id, _scope_for_line(node.lineno))
                if sc_idx is not None:
                    scope_var_types.setdefault(sc_idx, {}).setdefault(
                        tgt.id, []
                    ).append((node.lineno, type_name))
            elif not isinstance(val, ast.Name):
                sc_idx = _binding_scope_for(tgt.id, _scope_for_line(node.lineno))
                if sc_idx is not None:
                    scope_var_types.setdefault(sc_idx, {}).setdefault(
                        tgt.id, []
                    ).append((node.lineno, None))
            _cet_elts = None
            ...
            if _cet_elts:
                ...
                for _cet_el in _cet_elts:
                    ...
                    if _cet_t is None or (
                        _cet_name is not None and _cet_t != _cet_name
                    ):
                        _cet_ok = False
                        break
                    _cet_name = _cet_t
                if (
                    _cet_ok
                    and _cet_name is not None
                    and (
                        _cet_name in local_classes
                        or _class_origin_at(_cet_name, node.lineno) is not None
                    )
                ):
                    _cet_sc = _binding_scope_for(tgt.id, _scope_for_line(node.lineno))
                    if _cet_sc is not None:
                        container_elem_types.setdefault(_cet_sc, {}).setdefault(
                            tgt.id, []
                        ).append((node.lineno, _cet_name))
    for _fn in ast.walk(tree):
        if not isinstance(_fn, (ast.For, ast.AsyncFor)) or not isinstance(
            _fn.target, ast.Name
        ):
            continue
        _fit = None
        if isinstance(_fn.iter, ast.Name):
            ...
            _fcands = (
                container_elem_types.get(_fsc0, {}).get(_fn.iter.id)
                if _fsc0 is not None
                else None
            )
            if _fcands:
                _fb = None
            ...
            if _fok:
                _fit = _fname
        if _fit is not None and (
            _fit in local_classes or _class_origin_at(_fit, _fn.lineno) is not None
        ):
            _ftsc = _binding_scope_for(_fn.target.id, _scope_for_line(_fn.lineno))
            if _ftsc is not None:
                scope_var_types.setdefault(_ftsc, {}).setdefault(
                    _fn.target.id, []
                ).append((_fn.lineno, _fit))
    for _valn, _vatgt, _vasrc in sorted(_var_type_alias_assigns):
        ...
        if _vtsc is not None:
            scope_var_types.setdefault(_vtsc, {}).setdefault(_vatgt, []).append(
                (_valn, _vtype)
            )
    _ck()
    for cls_node in tree_class_defs:
        for meth in cls_node.body:
            ...
            _fp = (
                meth.args.args[0].arg
                if (meth.args.args and _python_method_has_implicit_first_param(meth))
                else None
            )
            if not _fp:
                continue
            for stmt in ast.walk(meth):
                ...
                for tgt, val in _assign_pairs(stmt.targets, stmt.value):
                    if (
                        ...
                        continue
                    _self_assigned_attrs.setdefault(
                        _node_class_key(cls_node), set()
                    ).add(tgt.attr)
                    type_name = None
                    if isinstance(val, ast.Call):
                        func = val.func
                        ...
                        if isinstance(func, ast.Name):
                            type_name = func.id
                            mod_name = _line_def_at(
                                _from_import_map.get(type_name), stmt.lineno
                            )
                            if mod_name is None and not _name_is_class_at(
                                type_name, stmt.lineno
                            ):
                                type_name = None
                        elif isinstance(func, ast.Attribute):
                            type_name = func.attr
                            mod_name = (
                                _base_module_at(func.value.id, stmt.lineno)
                                if isinstance(func.value, ast.Name)
                                else None
                            )
                        if type_name and type_name not in local_classes and mod_name:
                            mems = _python_resolve_module_class_members(
                                buf, mod_name, type_name
                            )
                            if mems:
                                local_classes[type_name] = mems
                                class_module_origin.setdefault(type_name, []).append(
                                    (stmt.lineno, (mod_name, type_name))
                                )
                    elif isinstance(val, ast.Constant):
                        type_name = type(val.value).__name__
                    elif isinstance(val, ast.List):
                    ...
                    if type_name and type_name in local_classes:
                        ...
                        class_type_maps.setdefault(_node_class_key(cls_node), {})[
                            tgt.attr
                        ] = type_name
                    else:
                        class_type_maps.get(_node_class_key(cls_node), {}).pop(
                            tgt.attr, None
                        )
    for cls_node in tree_class_defs:
        ...
        for stmt in cls_node.body:
            ...
            if type_name and type_name in local_classes:
                ...
                for tgt in stmt.targets:
                    if isinstance(
                        tgt, ast.Name
                    ) and tgt.id not in _self_assigned_attrs.get(
                        _node_class_key(cls_node), ()
                    ):
                        class_type_maps.setdefault(_node_class_key(cls_node), {})[
                            tgt.id
                        ] = type_name
    ...
    for _cnode in tree_class_defs:
        if _cnode.name in local_classes:
            ...
            for _meth in _cnode.body:
                if (
                    isinstance(_meth, (ast.FunctionDef, ast.AsyncFunctionDef))
                    and _meth.args.args
                    and _python_method_has_implicit_first_param(_meth)
                ):
                    method_fp_ranges.append(
                        (
                            _meth.lineno,
                            getattr(_meth, "end_lineno", _meth.lineno),
                            _meth.args.args[0].arg,
                            _cnk,
                        )
                    )
    ...
    def _module_attr_kind(mod, attr):
        ...
        if (
            _spec is not None
            and getattr(_spec, "submodule_search_locations", None)
            and _python_find_spec_cached(buf, _sub) is not None
        ):
            return "module"
        return None
    ...
    def _assign_value_kind(val, lineno):
        ...
        if (
            isinstance(val, ast.Call)
            and builder._dynamic_import_module(val) is not None
        ):
            return "module"
        if isinstance(val, ast.Name):
    ...
    def _value_type(val, lineno):
        ...
        if isinstance(val, ast.Name):
            ...
            if val.id in local_classes and _call_name_kind(val.id, lineno) in (
                None,
                "class",
            ):
        ...
        if isinstance(val, ast.Attribute) and isinstance(val.value, ast.Name):
            if _call_name_kind(val.value.id, lineno) in ("module", None):
                ...
                if _vm is not None:
                    ...
                    if _vmk == "module":
                        _vtgt = _python_resolve_module_members(buf, _vm).get(
                            "@modtarget:" + val.attr
                        )
                        return (
                            "module",
                            _vtgt if _vtgt is not None else f"{_vm}.{val.attr}",
                        )
            return None
        return None
    ...
    for _ccnode in tree_class_defs:
        ...
        for _cstmt in _flatten_class_body(_ccnode.body):
            _ctgts = (
                _cstmt.targets
                if isinstance(_cstmt, ast.Assign)
                else ([_cstmt.target] if isinstance(_cstmt, ast.AnnAssign) else [])
            )
            for _ctgt, _cval in _assign_pairs(_ctgts, _cstmt.value if _ctgts else None):
                ...
                if _cvt is not None:
                    class_attr_types.setdefault(_node_class_key(_ccnode), {})[
                        _ctgt.id
                    ] = _cvt
    ...
    for _ucnode in tree_class_defs:
        ...
        for _umeth in _ucnode.body:
            if (
                not isinstance(_umeth, (ast.FunctionDef, ast.AsyncFunctionDef))
                or not _umeth.args.args
                or not _python_method_has_implicit_first_param(_umeth)
            ):
                continue
            ...
            for _ustmt in ast.walk(_umeth):
                _utgts = (
                    _ustmt.targets
                    if isinstance(_ustmt, ast.Assign)
                    else ([_ustmt.target] if isinstance(_ustmt, ast.AnnAssign) else [])
                )
                for _utgt, _uval in _assign_pairs(
                    _utgts, _ustmt.value if _utgts else None
                ):
                    if not (
                        isinstance(_utgt, ast.Attribute)
                        and isinstance(_utgt.value, ast.Name)
                        and _utgt.value.id == _ufp
                    ):
                        continue
                    ...
                    if (
                        _ukind is not None
                        and local_classes[_unk].get(_utgt.attr) == "var"
                    ):
                        local_classes[_unk][_utgt.attr] = _ukind
                    _uvt = (
                        _value_type(_uval, _ustmt.lineno) if _uval is not None else None
                    )
                    if _uvt is not None:
    ...
    def _infer_type(node):
        ...
        if isinstance(node, ast.Call):
            if (
                ...
                and _call_name_kind("__import__", node.lineno) is None
                and node.args
                and isinstance(node.args[0], ast.Constant)
                and isinstance(node.args[0].value, str)
            ):
                if (
                    node.args[0].value in valid_modules
                    or _python_find_spec_cached(buf, node.args[0].value) is not None
                ):
            ...
            if (
                ...
                and _call_name_kind("super", node.lineno) is None
            ):
                _sbest = None
                for _ms, _me, _mfp, _mcls in method_fp_ranges:
                    if _ms <= node.lineno <= _me and (
                        _sbest is None or _ms > _sbest[0]
                    ):
                        _sbest = (_ms, _mcls)
            ...
            if isinstance(node.func, ast.Attribute):
                ...
                if br is not None and br[0] in ("instance", "class"):
                    ...
                    if rt is None:
                        rt = _PYTHON_BUILTIN_METHOD_RETURNS.get(br[1], {}).get(
                            node.func.attr
                        )
        ...
        if isinstance(node, ast.Attribute):
            ...
            if (
                ...
                and _fp_class_at(node.value.id, node.lineno) != r[1]
            ):
                _vasc = _binding_scope_for(node.value.id, _scope_for_line(node.lineno))
                if _vasc is not None:
                    _vfound, _vtype = _var_attr_type_at(
                        _vasc, node.value.id, node.attr, node.lineno
                    )
                    if _vfound:
                        return _vtype
            if r[0] == "module":
                ...
                if _mk == "module":
                    _tgt = _python_resolve_module_members(buf, r[1]).get(
                        "@modtarget:" + node.attr
                    )
                    return (
                        "module",
                        _tgt if _tgt is not None else f"{r[1]}.{node.attr}",
                    )
                if _mk == "class":
    ...
    for _an in sorted(tree_assigns, key=lambda _n: (_n.lineno, _n.col_offset)):
        for _atgt, _aval in _assign_pairs(_an.targets, _an.value):
            ...
            if (
                isinstance(_atgt.value, ast.Name)
                and _call_name_kind(_atgt.value.id, _an.lineno) == "class"
                and _atgt.value.id in local_classes
                and _class_origin_at(_atgt.value.id, _an.lineno) is None
            ):
                ...
                if _adkind == "class":
                    ...
                    if _avr is not None and _avr[0] in ("class", "modclass"):
                        class_attr_types.setdefault(_atgt.value.id, {})[
                            _atgt.attr
                        ] = _avr
            ...
            if _br[0] == "module":
                ...
                if _atn is not None and _atn in local_classes:
                    dynamic_modclass_attr_types.setdefault(_br[2], {})[
                        _atgt.attr
                    ] = _atn
                ...
                _avt = _value_type(_aval, _an.lineno) if _aval is not None else None
                _is_self_attr = (
                    not isinstance(_atgt.value, ast.Name)
                    or _fp_class_at(_atgt.value.id, _an.lineno) == _br[1]
                )
                if not _is_self_attr:
                    _vasc = _binding_scope_for(
                        _atgt.value.id, _scope_for_line(_an.lineno)
                    )
                    if _vasc is not None:
                        if _avt is not None:
                            _vaval = _avt
                            ...
                            _vaval = None
                        var_attr_types.setdefault(
                            (_vasc, _atgt.value.id), {}
                        ).setdefault(_atgt.attr, []).append((_an.lineno, _vaval))
    ...
    for node in tree_assigns:
        ...
        for tgt, _tv in _assign_pairs(node.targets, node.value):
            ...
            if _ir is not None and _ir[0] == "instance" and _ir[1] in local_classes:
                scope_var_types.setdefault(_isc, {}).setdefault(tgt.id, []).append(
                    (node.lineno, _ir[1])
                )
    ...
    for _ in range(10):
        ...
        for _asc, _aln, _aname, _asrc in builder.alias_assigns:
            ...
            _aln_names = builder.scopes[_binding_scope_for(_aname, _asc)][
                "names"
            ].setdefault(_aname, [])
            _aln_names[:] = [
                (_l, _k) for _l, _k in _aln_names if not (_l == _aln and _k == "var")
            ]
    ...
    for _mcb_key, _mcb_node in _module_class_bases:
        ...
        local_class_module_origins.setdefault(_mcb_key, []).append(
            (_mcb_r[1], _mcb_r[2])
        )
        ...
        for _lcmo_base in _lcmo_node.bases:
            if isinstance(_lcmo_base, ast.Name):
                ...
                if _lcmo_orig is not None:
                    local_class_module_origins.setdefault(_lcmo_key, []).append(
                        _lcmo_orig
                    )
    for _ in range(10):
        _lcmo_changed = False
            ...
            for _lcmo_b in _lcmo_bases:
                for _lcmo_o in local_class_module_origins.get(_lcmo_b, []):
                    if _lcmo_o not in local_class_module_origins.get(_lcmo_cls, []):
                        local_class_module_origins.setdefault(_lcmo_cls, []).append(
                            _lcmo_o
                        )
                        _lcmo_changed = True
    ...
    for node in tree_attributes:
        ...
        if _k == "var" and r[0] != "builtin":
            ...
            if _kir is not None and (
                _kir[0] == "minstance"
                or (_kir[0] == "instance" and _kir[1] not in _PYTHON_BUILTIN_MEMBERS)
            ):
                _k = "instance"
        if _k is not None:
            typed_attrs.append(
                (node.end_lineno, node.end_col_offset - len(node.attr), node.attr, _k)
            )
    _ck()
    var_module_class = {}
    for _vn in tree_assigns:
        ...
        if isinstance(_fn, ast.Attribute) and isinstance(_fn.value, ast.Name):
            ...
            if (
                _bmod is not None
                and _python_resolve_module_members(buf, _bmod).get(_fn.attr) == "class"
            ):
                _cmod = _bmod
                _ccls = _fn.attr
            ...
            if (
                _ffm is not None
                and _python_resolve_module_members(buf, _ffm[0]).get(_ffm[1]) == "class"
            ):
                _cmod, _ccls = _ffm
            else:
        ...
        for _t in _vn.targets:
            if isinstance(_t, ast.Name):
                var_module_class.setdefault(
                    (_binding_scope_for(_t.id, _vsc), _t.id), []
                ).append((_vn.lineno, _cmod, _ccls))
    ...
    for lineno, func_name, kwarg_name, is_name, dotted, _cnode in builder.pending_calls:
            ...
            if not ok:
                ...
                if _lcls is not None:
                    ...
                    if _lkey in local_class_accepts_any:
                        ...
                        found, mp = _python_resolve_module_method(
                            buf, _lorig[0], _lorig[1], func_name
                        )
                        if found:
                            ok, params = True, mp
                    if not ok:
                        for _lcmo_mod, _lcmo_cls in local_class_module_origins.get(
                            ...
                            found, mp = _python_resolve_module_method(
                                buf, _lcmo_mod, _lcmo_cls, func_name
                            )
                            if found:
                                ok, params = True, mp
                                break
        if (
            not ok
            and isinstance(_cnode.func, ast.Attribute)
            and isinstance(_cnode.func.value, ast.Call)
            and isinstance(_cnode.func.value.func, ast.Name)
            and _cnode.func.value.func.id == "super"
            and _call_name_kind("super", lineno) is None
        ):
            _super_cls = None
            for _sms, _sme, _smfp, _smcls in method_fp_ranges:
                if _sms <= lineno <= _sme and (
                    _super_cls is None or _sms > _super_cls[0]
                ):
            ...
            if _super_cls is not None:
                for _sbase in class_bases.get(_super_cls[1], []):
                    ...
                    if _sbkey in local_class_method_params:
                        ...
                        break
                if not ok:
                    for _scmo_mod, _scmo_cls in local_class_module_origins.get(
                        _super_cls[1], []
                    ):
                        found, mp = _python_resolve_module_method(
                            buf, _scmo_mod, _scmo_cls, func_name
                        )
                        if found:
                            ok, params = True, mp
                            break
        if not ok and isinstance(_cnode.func, ast.Attribute):
            ...
            if _rcv is not None:
                if _rcv[0] in ("modclass", "minstance"):
                    ...
                    if _mok:
                        ok, params = _mok, _mp
                elif _rcv[0] in ("instance", "class") and not (
                    _rcv[0] == "class" and _instance_only_attr(_rcv[1], func_name)
                ):
                    if local_classes.get(_rcv[1], {}).get(func_name) == "class":
                        ...
                        if (
                            ...
                            if _tkey + ".__init__" in local_class_accepts_any:
                                ...
                                ok, params = (
                                    True,
                                    local_class_method_params[_tkey + ".__init__"],
                                )
                            else:
                                ok, params = True, set()
                                for (
                                    _lcmo_mod,
                                    _lcmo_cls,
                                ) in local_class_module_origins.get(_tkey, []):
                                    found, mp = _python_resolve_module_method(
                                        buf, _lcmo_mod, _lcmo_cls, "__init__"
                                    )
                                    if found:
                    ...
                    if not ok and _lkey in local_class_accepts_any:
                        ...
                        found, mp = _python_resolve_module_method(
                            buf, _rorig[0], _rorig[1], func_name
                        )
                        if found:
                            ok, params = True, mp
                    if not ok:
    ...
    for _cdk_ln, _cdk_cls, _cdk_defln, _cdk_arg in builder.class_def_kwargs:
        ...
        for _cdk_base in class_bases.get(_cdk_key, []):
            ...
            for _cdk_mod, _cdk_mcls in local_class_module_origins.get(_cdk_base, []):
                found, mp = _python_resolve_module_method(
                    buf, _cdk_mod, _cdk_mcls, "__init_subclass__"
                )
                if found:
        ...
        for dexpr in defaults:
            for nm in ast.walk(dexpr):
                if isinstance(nm, ast.Name) and nm.id in param_names:
                    ...
                    if kind is not None:
                        param_default_tags.append(
                            (nm.lineno, nm.col_offset, nm.id, kind)
                        )
    _ck()
    literal_attrs = []
    for node in tree_attributes:
        ...
        if _lat and node.attr in _PYTHON_BUILTIN_MEMBERS.get(_lat, {}):
            literal_attrs.append(
                (node.end_lineno, node.end_col_offset - len(node.attr), node.attr, _lat)
            )
    _ck()
    instance_name_positions = set()
    _instance_name_cache = {}
    for _inl, _incol, _inname, _instore in list(name_positions) + [
        (_pl, _pcol, _pname, False) for _pl, _pcol, _pname, _pkind in param_default_tags
    ]:
        ...
        if _inres is None:
            ...
            _inres = (
                _invt is not None
                and _invt in local_classes
                and _invt not in _PYTHON_BUILTIN_MEMBERS
            ) or _var_modclass_at(_inname, _inl, _incol) is not None
    ...
    for _gsl, _gscol, _gsname, _gsscope in global_stmt_positions:
        ...
        if _gskind == "var":
            ...
            if (
                _gsvt is not None
                and _gsvt in local_classes
                and _gsvt not in _PYTHON_BUILTIN_MEMBERS
            ):
                _gskind = "instance"
            elif (
                _gs_best(
                    [
                        (_d[0], (_d[1], _d[2]))
                        for _d in var_module_class.get((_gstgt, _gsname), ())
                    ]
                )
                is not None
            ):
    ...
    _export_class_bases = {
        _k: _v for _k, _v in class_bases.items() if _k in module_scope_class_keys
    }
    ...
    _export_inherited = {
        "members": _inherited_members,
        "attr_types": _inherited_attr_types,
        "method_params": _inherited_method_params,
    }
    return (
        builder.scopes,
        call_kwargs,
        builder.module_alias_defs,
        local_classes,
        module_literals,
        scope_var_types,
        literal_attrs,
        ...
        typed_attrs,
        param_default_tags,
        builder.kwarg_positions,
        import_dotted_lines,
        import_orig_name_tags,
        class_module_origin,
        local_class_method_params,
        local_class_accepts_any,
        name_positions,
        local_class_module_origins,
        from_func_module,
        class_type_maps,
        ...
        _export_class_bases,
        _export_inherited,
        module_scope_class_keys,
        _export_instance_only,
        instance_name_positions,
        global_stmt_kind_positions,
    )


def _python_scan_names(buf, text, gen=None):
    """Run _python_build_scopes() for an Editor buffer (not the Python
    Shell, which manages its own session-carrying scan in
    _hapyshell_body() instead - this uses no seed_* state) and store
    the (name/type/import/etc.) results directly on buf's
    _python_* attributes, then queue a re-highlight. Called on a
    background thread by _python_scan_start(); does nothing if the
    scan was cancelled (buffer changed mid-scan) or otherwise failed."""
...
def _python_scan_start(buf):
    """Start a background scan (_python_scan_names()) of buf's current
    content, deferring (retrying every 10ms) if a previous scan is
    still running rather than starting a second one concurrently.
    Called (debounced) by Editor.python_trigger_name_scan() after an
    edit."""
    buf._python_scan_after_id = None
    if (
        buf._python_names_scan_thread is not None
        and buf._python_names_scan_thread.is_alive()
    ):
        buf._python_scan_after_id = buf._own_type.after(
            10, lambda: _python_scan_start(buf)
        )
        return
...
def _python_resolve_module_func_params(buf, name):
    """Statically parse module name's source and build a map of every
    module-level function's and class's __init__/methods' valid
    keyword-argument names (None meaning "accepts any", i.e. it has
    **kwargs) - keyed "func" for plain functions, "Class" for a
    class's own __init__, "Class.method" for its methods, and
    "@bases:Class" for its (unresolved) base-class references - plus
    "@imports", a {local_name: (module, orig_name_or_None)} map of
    everything name imports, used to chase a class/function reference
    through an import to resolve it. `from x import *` re-exports
    are folded in afterward from the starred module's own map, without
    overwriting anything name already defines locally. Cached per
    (buffer, module name) in buf._python_module_func_params_cache."""
...
def _python_resolve_module_method(buf, mod, class_name, method, seen=None):
    """Resolve method's valid keyword arguments on mod.class_name,
    searching its own methods first, then (through an alias or base
    class, the same chasing as _python_resolve_module_member_kind())
    its ancestors. Returns (found, params); params is None if found
    but the method accepts arbitrary kwargs."""
    if seen is None:
        seen = set()
    key = (mod, class_name)
    if key in seen:
    ...
    if (
        ...
        return _python_resolve_module_method(
            buf, imports[class_name][0], imports[class_name][1], method, seen
        )
    for base in fp.get("@bases:" + class_name, []):
        bparts = base.split(".")
        if len(bparts) == 1:
            if "@bases:" + base in fp:
                ...
                if bname is not None:
                    ok, params = _python_resolve_module_method(
                        buf, bmod, bname, method, seen
                    )
                    if ok:
                        return True, params
        else:
            ...
            if broot in imports:
                ...
                full_mod = ".".join([bmod] + bparts[1:-1]) if len(bparts) > 2 else bmod
                ok, params = _python_resolve_module_method(
                    buf, full_mod, bparts[-1], method, seen
                )
                if ok:
                    return True, params
    return False, None
...
def _python_resolve_dotted_module(buf, dotted):
    """Resolve a dotted name like "a.b.c" to the real dotted module
    path it refers to, walking one component at a time and following
    any re-export aliasing along the way (via each package's
    "@modtarget:" entries from _python_resolve_module_members());
    returns None if any component isn't actually a module."""
    parts = dotted.split(".")
    cur = parts[0]
...
# Highlight patterns for Python keywords (excluding one used right
# after a dot, e.g. the "class" in "x.class") and operators, used
# directly by editor.py's/pythonshell.py's ha()/_hapyshell_body().
_PYTHON_KW_PAT = re.compile(
    r"(?<!\.)\b(?:" + "|".join(re.escape(k) for k in keyword.kwlist) + r")\b"
)
_PYTHON_OP_PAT = re.compile(
    r"\*\*=|//=|<<=|>>=|:=|==|!=|<=|>=|\+=|-=|\*=|/=|%=|&=|\|=|\^=|@=|->|\*\*|//|<<|>>|[+\-*/%@&|^~=<>]"
)


def _python_bytecol_to_charcol(line_str, bytecol):
    """Convert a UTF-8 byte offset within line_str (as ast gives column
    offsets for non-ASCII source) to a character offset (as tkinter
    text indices need)."""
    if bytecol <= 0:
        return bytecol
