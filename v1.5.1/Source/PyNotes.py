...
if platform.system() == 'Linux':
	...
	monospace = 'Courier'
class typing_textbox(tkinter.Text):
	def __init__(self, master, text, *args, **kwargs):
		super().__init__(master, *args, **kwargs)
		self.done = False
		self.curpos = '1.0'
		self.insert('end', text.replace('\n', '\u200b\n') + '\u200b')
		self.tag_add('cursor', '1.0', '1.1')
		self.tag_config('cursor', background = 'black', foreground = 'white')
		self.bind('<KeyPress>', self.keypress)
		self.config(state = 'disabled')
	def cursor(self, pos):
		self.curpos = pos
		self.tag_remove('cursor', '1.0', 'end')
		self.tag_add('cursor', pos, f'{pos}+1c')
		self.tag_config('cursor', background = 'black', foreground = 'white')
		self.see(pos + '+3l')
		self.see(pos)
	def keypress(self, event):
		...
		key = event.keysym
		if key in ('Delete', 'Shift_L', 'Shift_R', 'Escape', 'Tab', 'Caps_Lock', 'Control_L', 'Control_R', 'Alt_L', 'Alt_R', 'Super_L', 'Left', 'Right', 'Up', 'Down') or key in (f'F{i}' for i in range(1, 13)):
			self.config(state = 'disabled')
			self.cursor(self.curpos)
			return
		self.done = False
		if self.index(self.curpos) == self.index('end-2c') and key != 'BackSpace':
			self.cursor(self.curpos)
			self.config(state = 'disabled')
			return
		self.cursor(self.curpos)
		index = self.curpos
		if key == 'BackSpace':
			if self.get(f'{index}-1c', index) == '\n':
				index = f'{index}-1c'
			self.cursor(f'{index}-1c')
			self.curpos = f'{index}-1c'
			self.tag_remove('correct', f'{index}-1c', index)
			self.tag_remove('wrong', f'{index}-1c', index)
			self.config(state = 'disabled')
			return
		if self.get(index, f'{index}+1c') == '\n':
			index = f'{index}+1c'
		self.curpos = index
		acttext = self.get(index, f'{index}+1c')
		if any((key == acttext, key == 'Return' and acttext == '\u200b', key == 'space' and acttext == ' ', key == 'space' and acttext == '\u200b', key == 'comma' and acttext == ',', key == 'period' and acttext == '.', key == 'less' and acttext == '<', key == 'greater' and acttext == '>', key == 'question' and acttext == '?', key == 'slash' and acttext == '/', key == 'backslash' and acttext == '\\', key == 'bar' and acttext == '|', key == 'bracketleft' and acttext == '[', key == 'bracketright' and acttext == ']', key == 'braceleft' and acttext == '{', key == 'braceright' and acttext == '}', key == 'parenleft' and acttext == '(', key == 'parenright' and acttext == ')', key == 'apostrophe' and acttext == '\'', key == 'quotedbl' and acttext == '"', key == 'semicolon' and acttext == ';', key == 'colon' and acttext == ':', key == 'exclam' and acttext == '!', key == 'at' and acttext == '@', key == 'numbersign' and acttext == '#', key == 'dollar' and acttext == '$', key == 'percent' and acttext == '%', key == 'asciicircum' and acttext == '^', key == 'ampersand' and acttext == '&', key == 'asterisk' and acttext == '*', key == 'minus' and acttext == '-', key == 'underscore' and acttext == '_', key == 'plus' and acttext == '+', key == 'equal' and acttext == '=', key == 'grave' and acttext == '`', key == 'asciitilde' and acttext == '~')):
			self.tag_add('correct', index, f'{index}+1c')
			self.tag_config('correct', foreground = 'green')
		else:
			self.tag_add('wrong', index, f'{index}+1c')
			self.tag_config('wrong', foreground = 'white', background = 'red')
			rangetext = self.get(f'{index}-1c', index)
			if any((key == rangetext, key == 'Return' and rangetext == '\u200b', key == 'space' and rangetext == ' ', key == 'space' and rangetext == '\u200b', key == 'comma' and rangetext == ',', key == 'period' and rangetext == '.', key == 'less' and rangetext == '<', key == 'greater' and rangetext == '>', key == 'question' and rangetext == '?', key == 'slash' and rangetext == '/', key == 'backslash' and rangetext == '\\', key == 'bar' and rangetext == '|', key == 'bracketleft' and rangetext == '[', key == 'bracketright' and rangetext == ']', key == 'braceleft' and rangetext == '{', key == 'braceright' and rangetext == '}', key == 'parenleft' and rangetext == '(', key == 'parenright' and rangetext == ')', key == 'apostrophe' and rangetext == '\'', key == 'quotedbl' and rangetext == '"', key == 'semicolon' and rangetext == ';', key == 'colon' and rangetext == ':', key == 'exclam' and rangetext == '!', key == 'at' and rangetext == '@', key == 'numbersign' and rangetext == '#', key == 'dollar' and rangetext == '$', key == 'percent' and rangetext == '%', key == 'asciicircum' and rangetext == '^', key == 'ampersand' and rangetext == '&', key == 'asterisk' and rangetext == '*', key == 'minus' and rangetext == '-', key == 'underscore' and rangetext == '_', key == 'plus' and rangetext == '+', key == 'equal' and rangetext == '=', key == 'grave' and rangetext == '`', key == 'asciitilde' and rangetext == '~')):
				index = f'{index}-1c'
				self.tag_remove('correct', index, f'{index}+2c')
				self.tag_remove('wrong', index, f'{index}+2c')
				self.tag_add('wrong', index, f'{index}+1c')
				self.tag_config('wrong', foreground = 'white', background = 'red')
			rangetext = self.get(f'{index}+1c', f'{index}+2c')
			if any((key == rangetext, key == 'Return' and rangetext == '\u200b', key == 'space' and rangetext == ' ', key == 'space' and rangetext == '\u200b', key == 'comma' and rangetext == ',', key == 'period' and rangetext == '.', key == 'less' and rangetext == '<', key == 'greater' and rangetext == '>', key == 'question' and rangetext == '?', key == 'slash' and rangetext == '/', key == 'backslash' and rangetext == '\\', key == 'bar' and rangetext == '|', key == 'bracketleft' and rangetext == '[', key == 'bracketright' and rangetext == ']', key == 'braceleft' and rangetext == '{', key == 'braceright' and rangetext == '}', key == 'parenleft' and rangetext == '(', key == 'parenright' and rangetext == ')', key == 'apostrophe' and rangetext == '\'', key == 'quotedbl' and rangetext == '"', key == 'semicolon' and rangetext == ';', key == 'colon' and rangetext == ':', key == 'exclam' and rangetext == '!', key == 'at' and rangetext == '@', key == 'numbersign' and rangetext == '#', key == 'dollar' and rangetext == '$', key == 'percent' and rangetext == '%', key == 'asciicircum' and rangetext == '^', key == 'ampersand' and rangetext == '&', key == 'asterisk' and rangetext == '*', key == 'minus' and rangetext == '-', key == 'underscore' and rangetext == '_', key == 'plus' and rangetext == '+', key == 'equal' and rangetext == '=', key == 'grave' and rangetext == '`', key == 'asciitilde' and rangetext == '~')):
				index = f'{index}+1c'
				self.tag_add('wrong', f'{index}-1c', f'{index}+1c')
				self.tag_config('wrong', foreground = 'white', background = 'red')
		if self.get(index, f'{index}+1c') == '\u200b':
			index = f'{index}+1c'
		if self.index(self.curpos) == self.index('end-3c'):
			self.done = True
		self.curpos = f'{index}+1c'
		self.cursor(self.curpos)
		self.config(state = 'disabled')
...
def faketerm(number):
	...
	if number == 0:
		...
		command = 'pip3 install python-chess'
	elif number == 7:
...
try:
	...
	ans = ask('Error!', 'The module \'python-chess\' is not installed. Should PyNotes install it locally?')
	if not ans:
		exit()
	else:
		faketerm(6)
...
try:
	...
	pass
try:
	os.mkdir(f'{homedir}/.local/PyNotes/')
except:
	pass
try:
	file = open(f'{homedir}/.local/PyNotes/defs', 'r')
except:
	file = open(f'{homedir}/.local/PyNotes/defs', 'w+')
	file.write(f"{v}\nFalse\n{monospace}\nbreeze\n{rootdir}/english.txt\nFalse\n'python:keywords': \"foreground = 'purple', font = (type_.cget('font'), 12, 'bold')\", 'python:inbuilt': \"foreground = 'red'\", 'python:comments': \"foreground = 'gray'\", 'python:quotes': \"foreground = 'blue'\", 'python:variables': \"foreground = 'brown'\", 'python:functions': \"foreground = 'brown'\", 'python:operators': \"background = 'gray'\", 'latex:inlinemath': \"foreground = 'green'\", 'latex:environment': \"background = 'light green'\", 'latex:comments': \"foreground = 'gray'\", 'html:attributes': \"foreground = 'red'\", 'html:tags': \"foreground = 'dark green'\", 'html:comments': \"foreground = 'gray'\", 'html:quotes': \"foreground = 'blue'\"")
	file.close()
	file = open(f'{homedir}/.local/PyNotes/defs', 'r')
...
try:
	...
	if defs[5] == 'False':
		...
		taborspace = True
	exec('theme = ' + '{' + defs[6].replace('\n', '') + '}')
	theme['python:variables']
	theme['python:operators']
	theme['python:functions']
except:
	truncate = root.ask('Warning', 'The old preferences are incompatible with this version of PyNotes.\nDo you want to truncate the old preferences and continue?', ('yes', 'no'))
	if not truncate:
		root.error('Error!', 'Quitting PyNotes')
		root.destroy()
		exit()
	os.remove(f'{homedir}/.local/PyNotes/defs')
	file = open(f'{homedir}/.local/PyNotes/defs', 'w+')
	file.write(f"{v}\nFalse\n{monospace}\nbreeze\n{rootdir}/english.txt\nFalse\n'python:keywords': \"foreground = 'purple', font = (type_.cget('font'), 12, 'bold')\", 'python:inbuilt': \"foreground = 'red'\", 'python:comments': \"foreground = 'gray'\", 'python:quotes': \"foreground = 'blue'\", 'python:variables': \"foreground = 'brown'\", 'python:functions': \"foreground = 'brown'\", 'python:operators': \"background = 'gray'\", 'latex:inlinemath': \"foreground = 'green'\", 'latex:environment': \"background = 'light green'\", 'latex:comments': \"foreground = 'gray'\", 'html:attributes': \"foreground = 'red'\", 'html:tags': \"foreground = 'dark green'\", 'html:comments': \"foreground = 'gray'\", 'html:quotes': \"foreground = 'blue'\"")
	file.close()
	file = open(f'{homedir}/.local/PyNotes/defs', 'r')
...
def chesspy():
	if platform.system() == 'Linux':
		os.system('python3 /usr/share/PyNotes/ChessPy.py &')
	else:
		os.system('START /B python "C:/Program Files/PyNotes/ChessPy.py"')
def ss():
...
def abt(event = None):
	...
	abw.text(master = abw_, text = 'PyNotes is a text editor made in Python\nChange the HMode using the hmode command with Alt-X\nand then F5 to run a python/latex/html program.', font = ('TkDefaultFont', 12)).grid(column = 0, row = 2, sticky = 'w')
...
def changes(event = None):
	...
	cw.text(master = cw_, text = '1. Added a typing test in the \'Typing\' Tab! (You can also switch to it by the Alt-X command \'tt\' or \'typing\'.)').grid(column = 0, row = 0, sticky = 'w')
	cw.text(master = cw_, text = '2. Fixed a few bugs.').grid(column = 0, row = 1, sticky = 'w')
	...
	if not nm == '':
		...
		try:
			...
			if os.path.splitext(nm)[1] == '.py':
				...
				for widget in lf.winfo_children()[1:]:
					widget.config(state = 'normal')
				filetype.config(text = 'HTML File (' + os.path.splitext(nm)[1] + ')')
			else:
...
def prf(event = None):
	...
	pr.check(master = gt, text = 'Python: Use spaces instead of tabs to auto-indent', command = lambda: setts(varts.get()), var = varts).grid(column = 0, row = 1, sticky = 'w')
	...
	showfont = pr.textbox(master = tft, font = type_.cget('font'), wrap = 'word', height = 5)
	showfont.insert('end', 'The quick brown fox jumped over the lazy dogs\nAaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz\n1234567890\n?.,<>;:\'"{}[]\\|\n!@#$%^&*()-_+=')
	showfont.grid(column = 0, row = 1)
	f = pr.droptype(options = tuple(sorted(pr.getfonts())), master = mf, command = lambda: [pr.sizabletrue(), type_.config(font = f.get()), ln.config(font = f.get()), showfont.config(font = f.get()), pr.sizablefalse()])
	...
	colours.insert('end', str(theme)[:-1][1:].replace('type_.cget(\'font\')', 'orgfont'))
...
def hpb():
	...
	for k in ('file', 'open', 'map', 'int', 'str', 'print', 'range', 'set', 'input', 'list', 'len', 'self', 'type', 'exec', 'sum', 'iter', 'dir', 'compile', 'eval', 'format', 'locals', 'cls', 'exit', 'quit', 'dict', 'repr', 'hasattr', 'setattr', 'super', 'isinstance', 'object', 'tuple', 'float', 'True', 'False'):
		n = '1.0'
		searchstr = r'\y' + k + r'\y'
		while True:
			n = type_.search(searchstr, n, nocase = 1, stopindex = 'end', regexp = True)
...
def hpd():
	...
	while True:
		...
		type_.tag_add('hpd', n, nn)
		n = nn
	exec("type_.tag_config('hpd'," + theme['python:quotes'] + ')')
def hla():
...
def hha():
	type_.tag_remove('hattr', '1.0', 'end')
	tags = ['class', 'id', 'href', 'src', 'alt', 'style', 'type', 'name', 'value']
	for tag in tags:
		n = '1.0'
		searchstr = tag
		while True:
			...
			if not n: break
			nn = '%s+%dc' % (n, count.get())
			type_.tag_add('hattr', n, nn)
			n = nn
	exec("type_.tag_config('hattr'," + theme['html:attributes'] + ')')
def hhs():
	type_.tag_remove('hstuff', '1.0', 'end')
	n = '1.0'
	searchstr = r'<.+?>'
	while True:
		count = root.intvar()
		...
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		type_.tag_add('hstuff', n, nn)
		n = nn
	exec("type_.tag_config('hstuff'," + theme['html:tags'] + ')')
def hhc():
	n = '1.0'
	searchstr = r'<\!.+?>'
	type_.tag_remove('hcmt', '1.0', 'end')
	while True:
		count = root.intvar()
		n = type_.search(searchstr, n, nocase = 1, count = count, stopindex = 'end', regexp = True)
		if not n: break
		...
		for tag in type_.tag_names():
			if tag == 'sel': continue
			type_.tag_remove(tag, n, nn)
		type_.tag_add('hcmt', n, nn)
		n = nn
	exec("type_.tag_config('hcmt'," + theme['html:comments'] + ')')
def hhstr():
	n = '1.0'
	searchstr = r"'.+?'"
	type_.tag_remove('hstr', '1.0', 'end')
	while True:
		count = root.intvar()
		n = type_.search(searchstr, n, nocase = 1, count = count, stopindex = 'end', regexp = True)
		if not n: break
...
def hpv():
	definedvars = []
	type_.tag_remove('hpv', '1.0', 'end')
	n = '1.0'
	searchstr = r'^\s*[a-zA-Z_][a-zA-Z0-9_]*\s*='
	while True:
		count = root.intvar()
		n = type_.search(searchstr, n, nocase = 1, count = count, stopindex = 'end', regexp = True)
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		definedvars.append(type_.get(n, nn + '-1c').strip())
		n = nn
	for var in definedvars:
		n = '1.0'
		searchstr = r'\y' + var + r'\y'
		while True:
			count = root.intvar()
			n = type_.search(searchstr, n, nocase = 1, count = count, stopindex = 'end', regexp = True)
			if not n: break
			nn = '%s+%dc' % (n, count.get())
			type_.tag_add('hpv', n, nn)
			n = nn
	exec("type_.tag_config('hpv'," + theme['python:variables'] + ')')
def hpf():
	definedfuncs = []
	type_.tag_remove('hpf', '1.0', 'end')
	n = '1.0'
	searchstr = r'^\s*def \s*[a-zA-Z_][a-zA-Z0-9_]*\s*\('
	while True:
		count = root.intvar()
		n = type_.search(searchstr, n, nocase = 1, count = count, stopindex = 'end', regexp = True)
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		definedfuncs.append(type_.get(n + '+4c', nn + '-1c').strip())
		n = nn
	for var in definedfuncs:
		n = '1.0'
		searchstr = r'\y' + var + r'\y'
		while True:
			count = root.intvar()
			...
			if not n: break
			nn = '%s+%dc' % (n, count.get())
			type_.tag_add('hpf', n, nn)
			n = nn
	exec("type_.tag_config('hpf'," + theme['python:functions'] + ')')
def hpo():
	n = '1.0'
	searchstr = r'==|\!=|<|>|<=|>='
	type_.tag_remove('hpo', '1.0', 'end')
	while True:
		count = root.intvar()
		n = type_.search(searchstr, n, nocase = 1, count = count, stopindex = 'end', regexp = True)
		if not n: break
...
def keypress(event = None):
	...
	if filetype.cget('text') == 'Python File (.py)':
		...
		if filetype.cget('text') == 'LaTeX File (.tex)':
			ha('latex')
		elif filetype.cget('text') == 'HTML File (.html)':
			ha('html')
	if title and not filetype.cget('text') in ['PNG Image (*.png)', 'PDF File (*.pdf)', 'EPUB File (*.epub)']:
		if type_.get('1.0', 'end').strip() != open(title, 'r').read().strip():
			...
			root.title('PyNotes - ' + os.path.basename(title))
def indent():
	if not filetype.cget('text') == 'Python File (.py)':
		return
...
def st():
	...
	dwin.button(master = bframe, text = 'Start Recording', command = lambda: threading.Thread(target = record).start()).grid(column = 0, row = 0, padx = 10, pady = 10, sticky = 'w')
...
def cmdrun(command):
	...
	if command == 'exit' or command == 'e':
		...
		tabs.select(0)
		type_.focus()
	elif command == 'typing' or command == 'tt':
		tabs.select(3)
		tttb.focus()
...
def ptb(event = None):
	type_.yview_scroll(-1, 'pages')
def pcdone(nc):
	open(f'{homedir}/.pynotes', 'w+').write(nc)
	pcread()
def say(string):
...
def pcread():
	...
	try:
		code = open(f'{homedir}/.pynotes', 'r').read().replace('\n', '').split(';')
	except:
		return
	cdt = ''
	startupcdt = ''
	...
	file = open(f'{homedir}/.pynotesstartup', 'w+')
...
def pc(event = None):
	...
	try:
		open(f'{homedir}/.pynotes', 'r')
	except:
		open(f'{homedir}/.pynotes', 'w+')
	codeedit.insert('end', open(f'{homedir}/.pynotes', 'r').read())
...
def helppycode():
	...
	root.image(master = gt, image = f'{rootdir}/Images/PYCODE1.png', imsize = (2, 2)).grid(column = 0, row = 1, padx = 10, pady = 10)
	root.image(master = gt, image = f'{rootdir}/Images/PYCODE2.png', imsize = (2, 2)).grid(column = 1, row = 1, padx = 10, pady = 10)
...
def saydonestuff():
	...
	if ttspdn > 100:
		...
		if ttspdn > 125:
			say = 'GOD SPEED!'
		if ttspdn > 200:
			say = 'ABOVE 200 WPM!!! DID YOU CHEAT?'
	elif ttspdn > 90:
		say = 'You\'ll reach 100 WPM soon!'
	elif ttspdn > 80:
		say = 'Not bad speed. Try for 90 WPM!'
	if ttaccn > 95:
		if ttaccn == 100:
			say += '\nPERFECT ACCURACY!!! DID YOU CHEAT?'
		elif ttaccn > 98:
			say += '\nGOD ACCURACY!'
		else:
			say += '\nGood Accuracy!'
	elif ttaccn > 85:
		say += '\nNot bad Accuracy. Try to type a bit slowly and accurately.'
	root.info('Results', f'Speed: {ttspdn}\nAccuracy: {ttaccn}%\nNumber of Characters Typed: {typed}/{len(tttb.get('1.0', 'end-2c').replace('\n', ''))}\n{say}')
def counttt():
	global ttcountn
	global ttcountid
	global typed
	...
	if tttb.done:
		...
		return
	ttcountn -= 1
	ttspdn = round(typed*15/(120-ttcountn))
	min = ttcountn // 60
	sec = ttcountn % 60
	if min < 10:
		min = f'0{min}'
	if sec < 10:
		sec = f'0{sec}'
	ttcount.config(text = f'{min}:{sec}')
	ttspd.config(text = f'Speed: {ttspdn} WPM')
	if not ttcountn == 0:
		ttcountid = tttb.after(1000, counttt)
	else:
		saydonestuff()
def normkp(event):
	global typed
	...
	tttb.keypress(event)
	key = event.keysym
	if not any((key in ('BackSpace', 'Delete', 'Shift_L', 'Shift_R', 'Escape', 'Tab', 'Caps_Lock', 'Control_L', 'Control_R', 'Alt_L', 'Alt_R', 'Super_L', 'Left', 'Right', 'Up', 'Down'), key in (f'F{i}' for i in range(1, 13)))):
		typed += 1
		right = ttright()
		ttaccn = round(right*100/(right+ttwrong()))
		ttacc.config(text = f'Accuracy: {ttaccn}%')
	if key == 'BackSpace':
		typed = max(0, typed - 1)
def ttright():
	count = 0
	index = '1.0'
	while True:
		n = tttb.tag_nextrange('correct', index, 'end')
		if not n:
			break
		start, end = n
		text = tttb.get(start, end)
		count += len(text)
		index = end
	return count
def ttwrong():
	...
	index = '1.0'
	while True:
		n = tttb.tag_nextrange('wrong', index, 'end')
		if not n:
			break
		start, end = n
		text = tttb.get(start, end)
...
def starttt(event):
	...
	if not any((key in ('BackSpace', 'Delete', 'Shift_L', 'Shift_R', 'Escape', 'Tab', 'Caps_Lock', 'Control_L', 'Control_R', 'Alt_L', 'Alt_R', 'Super_L', 'Left', 'Right', 'Up', 'Down'), key in (f'F{i}' for i in range(1, 13)))):
		typed += 1
		right = ttright()
		ttacc.config(text = f'Accuracy: {round(right*100/(right+ttwrong()))}%')
	if key == 'BackSpace':
		typed = max(0, typed - 1)
	ttcountid = tttb.after(1000, counttt)
	tttb.unbind('<KeyPress>')
	tttb.bind('<KeyPress>', normkp)
def resettt():
	global ttcountn
	global ttspdn
	global typed
	...
	ttaccn = 100
	tttb.done = False
	ttacc.config(text = 'Accuracy: 100%')
	tttb.tag_remove('correct', '1.0', 'end')
	tttb.tag_remove('wrong', '1.0', 'end')
	tttb.cursor('1.0')
	tttb.after_cancel(ttcountid)
	ttcountn = 120
	ttspdn = 0
	typed = 0
	ttspd.config(text = 'Speed: 0 WPM')
	min = ttcountn // 60
	sec = ttcountn % 60
	if min < 10:
		min = f'0{min}'
...
tabs.add(ttf, text = 'Typing')
ttcount = root.text(master = ttf, font = ('TkDefaultFont', 15))
min = ttcountn // 60
sec = ttcountn % 60
if min < 10:
	min = f'0{min}'
if sec < 10:
	sec = f'0{sec}'
ttcount.config(text = f'{min}:{sec}')
ttcount.pack(side = 'top', fill = 'x', expand = True, padx = 10, pady = 10)
ttspd = root.text(master = ttf, text = 'Speed: 0 WPM', font = ('TkDefaultFont', 15))
ttspd.pack(side = 'top', fill = 'x', expand = True, padx = 10, pady = 10)
ttacc = root.text(master = ttf, text = 'Accuracy: 100%', font = ('TkDefaultFont', 15))
ttacc.pack(side = 'top', fill = 'x', expand = True, padx = 10, pady = 10)
tttb = typing_textbox(ttf, ttt, font = monospace)
tttb.bind('<KeyPress>', starttt)
tttb.pack(fill = 'both', expand = True, padx = 10, pady = 10)
root.button(master = ttf, text = 'Reset', command = resettt).pack(fill = 'x', expand = True, padx = 10, pady = 10)
fileinfo = root.frame()
cmdlabel = root.text()
...
m.add_cascade(label = 'ChessPy', menu = cm)
cm.add_command(label = 'Start', command = chesspy)
...
def hpvps():
	definedvars = []
	shellcmd.tag_remove('hpv', '1.0', 'end')
	n = '1.0'
	searchstr = r'^\s*[a-zA-Z_][a-zA-Z0-9_]*\s*='
	while True:
		count = root.intvar()
		n = shellcmd.search(searchstr, n, nocase = 1, count = count, stopindex = 'end', regexp = True)
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		definedvars.append(shellcmd.get(n, nn + '-1c').strip())
		n = nn
	for var in definedvars:
		n = '1.0'
		searchstr = r'\y' + var + r'\y'
		while True:
			...
			if not n: break
			nn = '%s+%dc' % (n, count.get())
			shellcmd.tag_add('hpv', n, nn)
			n = nn
	exec("shellcmd.tag_config('hpv'," + theme['python:variables'] + ')')
def hpfps():
	definedfuncs = []
	shellcmd.tag_remove('hpf', '1.0', 'end')
	n = '1.0'
	searchstr = r'^\s*def \s*[a-zA-Z_][a-zA-Z0-9_]*\s*\('
	while True:
		count = root.intvar()
		...
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		definedfuncs.append(shellcmd.get(n + '+4c', nn + '-1c').strip())
		n = nn
	for var in definedfuncs:
		n = '1.0'
		searchstr = r'\y' + var + r'\y'
		while True:
			...
			if not n: break
			nn = '%s+%dc' % (n, count.get())
			shellcmd.tag_add('hpf', n, nn)
			n = nn
	exec("shellcmd.tag_config('hpf'," + theme['python:functions'] + ')')
def hpops():
	n = '1.0'
	searchstr = r'==|\!=|<|>|<=|>='
	shellcmd.tag_remove('hpo', '1.0', 'end')
	while True:
		count = root.intvar()
		n = shellcmd.search(searchstr, n, nocase = 1, count = count, stopindex = 'end', regexp = True)
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		pos = int(n.split('.')[1])
		for tag in shellcmd.tag_names():
			if tag in ('sel', 'prompt'): continue
			shellcmd.tag_remove(tag, n, nn)
		if not pos in {0, 1, 2}:
			shellcmd.tag_add('hpo', n, nn)
		n = nn
	exec("shellcmd.tag_config('hpo'," + theme['python:operators'] + ')')
def hpaps():
	shellcmd.tag_remove('hpa', '1.0', 'end')
	for k in keyword.kwlist:
		if k in ('True', 'False'):
			continue
		n = '1.0'
		searchstr = r'\y' + k + r'\y'
		while True:
			n = shellcmd.search(searchstr, n, nocase = 1, stopindex = 'end', regexp = True)
			if not n: break
			nn = '%s+%dc' % (n, len(k))
			shellcmd.tag_add('hpa', n, nn)
			n = nn
		exec("shellcmd.tag_config('hpa'," + theme['python:keywords'] + ')')
def hpbps():
	shellcmd.tag_remove('hpb', '1.0', 'end')
	for k in ('file', 'open', 'map', 'int', 'str', 'print', 'range', 'set', 'input', 'list', 'len', 'self', 'type', 'exec', 'sum', 'iter', 'dir', 'compile', 'eval', 'format', 'locals', 'cls', 'exit', 'quit', 'dict', 'repr', 'hasattr', 'setattr', 'super', 'isinstance', 'object', 'tuple', 'float', 'True', 'False'):
		n = '1.0'
		searchstr = r'\y' + k + r'\y'
		while True:
			...
			if not n: break
			nn = '%s+%dc' % (n, len(k))
			shellcmd.tag_add('hpb', n, nn)
			n = nn
	exec("shellcmd.tag_config('hpb'," + theme['python:inbuilt'] + ')')
def hpcps():
	n = '1.0'
	searchstr = r'#.+?\n'
	shellcmd.tag_remove('hpc', '1.0', 'end')
	while True:
		count = root.intvar()
		n = shellcmd.search(searchstr, n, nocase = 1, stopindex = 'end', regexp = True, count = count)
		if not n: break
		nn = '%s+%dc' % (n, count.get())
		for tag in shellcmd.tag_names():
			if tag in {'sel', 'prompt'}: continue
			shellcmd.tag_remove(tag, n, nn)
		shellcmd.tag_add('hpc', n, nn)
		n = nn
	exec("shellcmd.tag_config('hpc'," + theme['python:comments'] + ')')
def hpdps():
	n = '1.0'
	searchstr = r"'([^'\\]*(\\.[^'\\]*)*)'"
	shellcmd.tag_remove('hpd', '1.0', 'end')
	while True:
		count = root.intvar()
		n = shellcmd.search(searchstr, n, nocase = 1, stopindex = 'end', regexp = True, count = count)
		if not n: break
	...
	while True:
		...
		shellcmd.tag_add('hpd', n, nn)
		n = nn
	exec("shellcmd.tag_config('hpd'," + theme['python:quotes'] + ')')
def hapyshell():
	hpaps()
	hpbps()
	hpvps()
	...
	for i in range(2, lines - 1):
		for tag in shellcmd.tag_names():
			if tag in ('sel', 'prompt'):
				continue
			shellcmd.tag_remove(tag, f'{i}.0', f'{i}.end')
def shellpy():
	global shellcmd
	...
	def colourprompts():
		...
		for i in range(1, lines):
			if not shellcmd.get(f'{i + 1}.0', f'{i + 1}.{lenprompt}') in {'>>> ', '... '}:
				continue
			shellcmd.tag_add('prompt', f'{i + 1}.0', f'{i + 1}.{lenprompt}')
	...
	def check_edit():
		...
		shellcmd.tag_remove('pytextinfo', '1.0', 'end')
		shellcmd.tag_add('pytextinfo', '1.0', f'1.{len(pytextinfo)}')
		shellcmd.tag_config('pytextinfo', foreground = 'blue', font = (monospace, 13, 'bold'))
		colourprompts()
		shellcmd.see('end')
	def exec(event):
		global readonlyendforshellpy
	...
	def cs():
		...
		readonlytextforshellpy = f'Python {sys.version} ({sys.executable})'.replace('\n', '') + '\n>>> '
		readonlyendforshellpy = '2.' + str(len('>>> '))
		pytextinfo = f'Python {sys.version} ({sys.executable})'.replace('\n', '') + '\n>>> '
		shellcmd.insert('end', pytextinfo)
		for tag in shellcmd.tag_names():
			shellcmd.tag_remove(tag, '1.0', 'end')
		shellcmd.tag_add('pytextinfo', '1.0', f'1.{len(pytextinfo)}')
		shellcmd.tag_config('pytextinfo', foreground = 'blue', font = (monospace, 13, 'bold'))
		colourprompts()
	...
	clearshell.pack(anchor = 'sw')
	shellcmd.tag_add('pytextinfo', '1.0', f"1.{len(f'Python {sys.version} ({sys.executable})'.replace('\n', ''))}")
	shellcmd.tag_config('pytextinfo', foreground = 'blue', font = (monospace, 13, 'bold'))
	shellcmd.bind('<KeyRelease>', lambda event: hapyshell())
	colourprompts()
shellpy()
def mathgod():
	if platform.system() == 'Linux':
		...
		os.system('START /B python "C:/Program Files/PyNotes/MathGod.py"')
def game():
	def start():
		gamewin.destroy()
		Game()
...
def emailsetup(saved = None):
	...
	def changeinfo():
		def emailsetupother():
			...
			s = server.get()
			po = port.get()
			file = open(f'{homedir}/.pynotesemailconfig', 'w+')
			file.write(f'{e}\n{p}\n{s}\n{po}')
			file.close()
	...
	if not saved:
		...
		encryptdecrypt(f'{homedir}/.pynotesemailconfig')
		file = open(f'{homedir}/.pynotesemailconfig', 'r').read().split('\n')
		encryptdecrypt(f'{homedir}/.pynotesemailconfig')
		e = file[0]
		p = file[1]
		s = file[2]
...
try:
	...
	root.error('Error', error)
try:
	open(f'{homedir}/.pynotesemailconfig', 'r')
except:
	loginframe = root.frame(master = ef)
	loginframe.pack(expand = True)
	root.text(master = loginframe, text = 'Email:').grid(column = 0, row = 0, padx = 10, pady = 10)
...
pcread()
try:
	file = open(f'{homedir}/.pynotesstartup', 'r').read().split('\n')
except:
	pass
else:
	for line in file:
