...
def changes(event = None):
    ...
    cw.text(master = cw_, text = '1. Fixed a few bugs, and finally added preferences saving!').grid(column = 0, row = 0, sticky = 'w')
    cw.text(master = cw_, text = '2. Added many more commands and basically made the whole thing even more like Emacs').grid(column = 0, row = 1, sticky = 'w')
    cw.text(master = cw__, text = 'IMPORTANT: A system like Elisp (Emacs) will probably come soon!', relief = 'sunken', padding = (10, 10, 10, 10)).grid(column = 0, row = 0, sticky = 'w')
...
type_ = root.textbox(master = mf, yscrollcommand = scrlbr.set, font = defs[2])
...
root.bind('<BackSpace>', lambda event: show('delete text'))
root.bind('<Delete>', lambda event: show('delete text'))
